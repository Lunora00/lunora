...
import {
  Code2, BarChart2, Brain, BookOpen,
  ChevronDown, Trash2, History,
  CheckCircle, Sparkles,
  Trophy, Target, TrendingUp, Clock,
  ArrowRight, RotateCcw, Info, Terminal,
} from "lucide-react";

// ─── AP Computer Science A 2026 Scoring ──────────────────────────────────────
// Section 1: 40 MCQ → 50% of composite · 1h 30min
// Section 2: 4 FRQ → 50% of composite · 1h 30min
//   Each FRQ worth 9 pts → total FRQ raw = 36 pts
// Composite = (MCQ/40 × 50) + (FRQ_raw/36 × 50) → /100
...
const FRQ_CONFIG = [
  { label: "Methods & Control Structures", max: 9 },
  { label: "Classes",                      max: 9 },
  { label: "Array / ArrayList / 2D Array", max: 9 },
  { label: "Recursion / Inheritance",      max: 9 },
];

const FRQ_TYPE_TAGS: Record<string, string> = {
  "Methods & Control Structures": "loops · conditionals · methods",
  "Classes":                      "OOP · constructors · encapsulation",
  "Array / ArrayList / 2D Array": "traversal · algorithms · data",
  "Recursion / Inheritance":      "recursion · polymorphism · extends",
};

const FRQ_TOTAL_RAW = 36;
const MCQ_TOTAL     = 40;
...
function getScoreMessage(score: number): string {
  return ({ 5: "Exceptional — you're on track for a 5. Your Java fundamentals and object-oriented thinking are solid.", 4: "Strong result. A 4 on AP CS A earns college credit at most universities.", 3: "You're in qualifying territory. Tighten up your array traversal and class design to push into a 4.", 2: "You're building the foundation. Focus on loops, methods, and object construction to move up fast.", 1: "Don't be discouraged. Consistent daily practice on Java fundamentals will turn this around." } as Record<number,string>)[score] ?? "";
}
function getThresholdRange(score: number): string {
  return ({ 5: "≥67", 4: "53–66", 3: "40–52", 2: "27–39", 1: "0–26" } as Record<number,string>)[score] ?? "";
}

interface SavedAttempt { id: string; mcq: number; frq: number[]; composite: number; apScore: number; savedAt: number; label: string; }
const DB_NAME = "lunora_ap_csa"; const DB_VERSION = 1; const STORE = "attempts";
function openDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => { const req = indexedDB.open(DB_NAME, DB_VERSION); req.onupgradeneeded = () => req.result.createObjectStore(STORE, { keyPath: "id" }); req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); });
}
async function saveAttempt(a: SavedAttempt): Promise<void> { try { const db = await openDB(); await new Promise<void>((res, rej) => { const tx = db.transaction(STORE, "readwrite"); tx.objectStore(STORE).put(a); tx.oncomplete = () => res(); tx.onerror = () => rej(tx.error); }); } catch { /* silent */ } }
async function loadAttempts(): Promise<SavedAttempt[]> { try { const db = await openDB(); return new Promise((res, rej) => { const tx = db.transaction(STORE, "readonly"); const req = tx.objectStore(STORE).getAll(); req.onsuccess = () => res(req.result || []); req.onerror = () => rej(req.error); }); } catch { return []; } }
async function deleteAttempt(id: string): Promise<void> { try { const db = await openDB(); await new Promise<void>((res, rej) => { const tx = db.transaction(STORE, "readwrite"); tx.objectStore(STORE).delete(id); tx.oncomplete = () => res(); tx.onerror = () => rej(tx.error); }); } catch { /* silent */ } }
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, userSelect: "none" }}>
      <svg viewBox="0 0 100 100" style={{ width: s, height: s, transform: "rotate(40deg)", flexShrink: 0 }}>
        <defs><mask id={`lunora-mask-${light ? "light" : "dark"}`}><rect width="100" height="100" fill="white" /><circle cx="57" cy="50" r="40" fill="black" /></mask></defs>
...
export default function APCSAScoreCalculator() {
  const [mcq, setMcq] = useState(26);
  const [frq, setFrq] = useState<number[]>([6, 6, 6, 6]);
  ...
  const faqs = [
    { q: "How is the AP Computer Science A exam scored in 2026?", a: "The AP Computer Science A exam has two sections. Section 1 is 40 multiple choice questions completed in 1 hour 30 minutes, worth 50% of your total score. Section 2 is 4 free response questions completed in 1 hour 30 minutes, worth the other 50%. Each FRQ is worth 9 points for a total raw FRQ score of 36. Your raw scores convert to a composite out of 100, which maps to an AP score of 1–5. The exam is entirely Java-based — no other language is permitted." },
    { q: "What composite score do I need for a 5 on AP Computer Science A?", a: "Based on recent College Board score distributions, you generally need a composite score of approximately 67 or above to earn a 5 on AP Computer Science A. Around 24–27% of test takers score a 5 each year, making it one of the more attainable 5s among STEM AP exams, particularly for students with prior programming experience." },
    { q: "What composite score do I need for a 3 on AP Computer Science A?", a: "A composite score of approximately 40 or above typically earns a 3 on AP Computer Science A. A 3 is considered 'qualified' and is accepted for introductory CS credit at many universities, though specific policies vary by school and department." },
    { q: "What are the four free response question types on AP Computer Science A?", a: "The four AP Computer Science A FRQ types are: (1) Methods and Control Structures — writing or completing methods using loops, conditionals, and basic Java syntax; (2) Classes — designing a complete class with instance variables, constructors, and methods; (3) Array/ArrayList/2D Array — traversing and manipulating data structures; (4) Recursion/Inheritance — writing recursive methods or extending classes using polymorphism. Each type appears on every exam and is worth 9 points." },
    { q: "What Java concepts are on AP Computer Science A?", a: "AP Computer Science A covers: primitive types (int, double, boolean), object-oriented programming (classes, objects, constructors, methods, encapsulation), inheritance and polymorphism, interfaces, arrays and ArrayLists, 2D arrays, String methods, Math class methods, iteration (for loops, while loops, for-each loops), recursion, and basic algorithm analysis. The College Board provides a Java Quick Reference sheet during the exam." },
    { q: "Is AP Computer Science A hard?", a: "AP Computer Science A has a moderate difficulty level with a 5 rate of roughly 24–27%, higher than most AP sciences. Students with prior programming experience in Java or another language tend to find it manageable. The main challenges are object-oriented design, recursion, and 2D array traversal. The FRQ section requires writing syntactically correct Java code from scratch under time pressure — a skill that only comes from practice." },
    { q: "What is the difference between AP Computer Science A and AP Computer Science Principles?", a: "AP Computer Science A (CSA) focuses on Java programming, object-oriented design, and data structures — it is technical and code-heavy. AP Computer Science Principles (CSP) is a broader conceptual course covering computing concepts, algorithms, data, the internet, and societal impacts, with less emphasis on a specific language. CSA is generally considered harder and more directly applicable to CS degree programs. Many universities give CS credit for a 4 or 5 on CSA but not CSP." },
    { q: "How can I improve my AP Computer Science A score?", a: "Master the four FRQ types by practicing each one individually. For Classes FRQs, practice designing complete classes from a specification. For Array/2D Array FRQs, drill traversal patterns including nested loops. For Recursion FRQs, trace through base cases and recursive calls by hand before writing code. Always write syntactically correct Java — missing semicolons and unclosed braces cost points. Use tools like Lunora to generate unlimited practice questions from your AP CS A notes and drill each topic until writing Java is second nature." },
  ];
    ...
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .frq-type-tag { display: inline-block; padding: 2px 8px; border-radius: 6px; background: rgba(74,124,199,0.08); color: #4a7cc7; font-size: 10px; font-weight: 600; letter-spacing: 0.04em; font-family: 'Courier New', monospace; margin-left: 8px; }
      `}</style>

      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP Computer Science A Score Calculator", "description": "Free AP Computer Science A score calculator for 2026. Enter your multiple choice and free response scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-computer-science-a-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [ { "@type": "Question", "name": "How is the AP Computer Science A exam scored?", "acceptedAnswer": { "@type": "Answer", "text": "AP CSA has two sections. Section 1 is 40 multiple choice questions worth 50% of your score. Section 2 is 4 free response questions (each 9 pts, total 36 raw pts) worth 50%. Raw scores convert to a composite out of 100, then to an AP score of 1–5." } }, { "@type": "Question", "name": "What composite score do I need for a 5 on AP Computer Science A?", "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 67 or above to earn a 5 on AP Computer Science A. About 24–27% of students score a 5 each year." } }, { "@type": "Question", "name": "What are the four FRQ types on AP Computer Science A?", "acceptedAnswer": { "@type": "Answer", "text": "The four FRQ types are: (1) Methods and Control Structures, (2) Classes, (3) Array/ArrayList/2D Array, and (4) Recursion/Inheritance. Each appears on every exam and is worth 9 points." } }, { "@type": "Question", "name": "What is the difference between AP Computer Science A and AP Computer Science Principles?", "acceptedAnswer": { "@type": "Answer", "text": "AP Computer Science A focuses on Java programming and object-oriented design. AP Computer Science Principles is a broader conceptual course covering computing concepts, the internet, and data. CSA is more technical and code-heavy." } } ] }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Terminal size={11} color="#203567" strokeWidth={2.5} />Free Tool · 2026</div>
          </div>
          ...
          <div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,53,103,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Code2 size={17} color="#203567" strokeWidth={2} /></div>
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Section 2: Free Response</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>4 questions · 9 pts each · 50% of total score · 1h 30min · Java only</p>
                </div>
              </div>
              ...
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>4 fixed FRQ types</strong> — each question type appears on every AP CS A exam. The College Board Java Quick Reference sheet is permitted during Section 2.
                </p>
              </div>
                ...
                {FRQ_CONFIG.map((q, i) => (
                  <div key={i}>
                    <div style={{ marginBottom: 8 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#203567" }}>Question {i + 1}</span>
                      <span className="frq-type-tag">{FRQ_TYPE_TAGS[q.label]}</span>
                    </div>
                    <ScoreSlider label={q.label} sublabel="9 pts" value={frq[i]} max={q.max} onChange={v => updateFrq(i, v)} color="#4a7cc7" />
                  </div>
                ))}
              </div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP Computer Science A score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 520, margin: "0 auto" }}>Every FRQ type is fixed and predictable — knowing the structure going in is a significant advantage.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "50% of total score", body: "40 multiple choice questions in 1 hour 30 minutes. Questions test Java concepts, code tracing, and algorithm analysis. No penalty for wrong answers.", formula: "MCQ / 40 × 50" },
              { Icon: Code2, color: "#4a7cc7", title: "Section 2: FRQ", sub: "50% of total score", body: "4 Java programming questions in 1 hour 30 minutes, each worth 9 points. The College Board Java Quick Reference sheet is permitted. Total raw FRQ = 36 points.", formula: "FRQ_raw / 36 × 50" },
              { Icon: Target, color: "#2d6e9e", title: "Fixed FRQ Types", sub: "same every year", body: "The four FRQ types never change: Methods & Control Structures, Classes, Array/ArrayList/2D Array, and Recursion/Inheritance. Each is worth 9 points every exam.", formula: "4 × 9 = 36 pts" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥67 → 5 · ≥53 → 4 · ≥40 → 3 · ≥27 → 2 · below → 1. About 24–27% of students earn a 5. Cut scores may shift slightly each year.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP CS A — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited AP CS A questions", body: "Generate practice questions on every FRQ type, Java concept, and algorithm from your own notes and study material." },
              { Icon: Brain, title: "Deep-dive on weak concepts", body: "Struggling with 2D array traversal or recursive methods? Get instant worked examples and mini quizzes on that exact topic." },
              { Icon: TrendingUp, title: "Track mastery per topic", body: "See best score, average score, and topic breakdown across every session. Know exactly which Java concepts you've locked in." },
      ...
      <section style={{ padding: "80px 24px", background: "#fff" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Code2 size={11} strokeWidth={2.5} />More AP Calculators</div></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Code2 size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP Computer Science A Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP Computer Science A is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP Computer Science A notes and labs into unlimited practice questions. Track your progress to a 5.</p>
