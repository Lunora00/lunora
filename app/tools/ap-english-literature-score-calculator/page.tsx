...
import {
  ...
  ArrowRight, RotateCcw, Info, BookMarked,
} from "lucide-react";

// ─── AP English Literature & Composition 2026 Scoring ─────────────────────
// Section 1: 55 MCQ → 45% of composite · 1h
// Section 2: 3 FRQ (essays) → 55% of composite · 2h
//   Essay 1: Poetry Analysis      /6
//   Essay 2: Prose Fiction Analysis /6
//   Essay 3: Literary Argument    /6  (open-ended, student chooses work)
//   Total FRQ raw = 18 pts
// Composite = (MCQ/55 × 45) + (FRQ_raw/18 × 55) → /100
// AP Score thresholds (approximate, College Board 2024–2025 data)
// AP English Literature: ~400k+ test takers
// 5 rate ~8–10%, passing rate ~55–60%
...
const FRQ_CONFIG = [
  {
    label: "Essay 1 — Poetry Analysis",
    max: 6,
    desc: "Read a provided poem and write an essay analyzing how the poet's literary choices (imagery, diction, syntax, structure, figurative language, tone) contribute to the poem's meaning. Scored on thesis (0–1), evidence/commentary (0–4), and sophistication (0–1).",
  },
  {
    label: "Essay 2 — Prose Fiction Analysis",
    max: 6,
    desc: "Read a provided prose passage (novel or short story excerpt) and write an essay analyzing how the writer's literary techniques contribute to meaning, character, or theme. Scored on thesis (0–1), evidence/commentary (0–4), and sophistication (0–1).",
  },
  {
    label: "Essay 3 — Literary Argument",
    max: 6,
    desc: "Respond to a provided prompt using a literary work of your choice. Write an argument about how a specific literary element (e.g., a character, symbol, or narrative choice) contributes to the work's overall meaning. Scored on thesis (0–1), evidence/commentary (0–4), and sophistication (0–1).",
  },
];

const FRQ_TOTAL_RAW = 18;
const MCQ_TOTAL     = 55;

function calcComposite(mcq: number, frq: number[]): number {
  const frqRaw = frq.reduce((a, b) => a + b, 0);
  return Math.round(((mcq / MCQ_TOTAL) * 45 + (frqRaw / FRQ_TOTAL_RAW) * 55) * 10) / 10;
}
...
function getScoreMessage(score: number): string {
  return ({
    5: "Outstanding — you've mastered close reading, literary analysis, and argumentation at the highest level.",
    4: "Strong result. A 4 on AP English Literature demonstrates genuine command of literary technique and analytical writing.",
    3: "You're in qualifying territory. Sharpen your thesis precision and line-level commentary to push into a 4.",
    2: "You're building the foundation. Focus on writing defensible theses and developing textual evidence with specific commentary.",
    1: "Don't be discouraged. Consistent close reading practice and timed essay writing will build the skills this exam tests.",
  } as Record<number, string>)[score] ?? "";
}
function getThresholdRange(score: number): string {
  return ({ 5: "≥72", 4: "58–71", 3: "44–57", 2: "30–43", 1: "0–29" } as Record<number, string>)[score] ?? "";
}

interface SavedAttempt { id: string; mcq: number; frq: number[]; composite: number; apScore: number; savedAt: number; label: string; }
const DB_NAME = "lunora_ap_lit"; const DB_VERSION = 1; const STORE = "attempts";
...
export default function APEnglishLiteratureScoreCalculator() {
  const [mcq, setMcq] = useState(33);
  const [frq, setFrq] = useState<number[]>([4, 4, 4]);
  ...
  const mcqComponent = Math.round((mcq / MCQ_TOTAL) * 45 * 10) / 10;
  const frqComponent = Math.round((frqRaw / FRQ_TOTAL_RAW) * 55 * 10) / 10;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP English Literature Score Calculator", "description": "Free AP English Literature score calculator for 2026. Enter your MCQ and essay scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-english-literature-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 580, margin: "0 auto 20px" }}>
            Enter your <strong style={{ color: "#1a1a1a" }}>Multiple Choice</strong> and <strong style={{ color: "#1a1a1a" }}>Essay</strong> scores to instantly see your predicted AP score of 1–5 and composite out of 100.
          </p>
          ...
          <div>
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>55 questions · <strong style={{ color: "#203567" }}>45%</strong> of total score · 1h</p>
                </div>
              </div>
              ...
              <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 12, color: "#888" }}>MCQ contributes to composite (45% weight)</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: "#203567" }}>{mcqComponent} / 45</span>
              </div>
            </div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Section 2: Free Response (Essays)</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>3 essays · 6 pts each · <strong style={{ color: "#203567" }}>55%</strong> of total score · 2h</p>
                </div>
              </div>
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Each essay: Thesis (1 pt) + Evidence & Commentary (0–4 pts) + Sophistication (1 pt).</strong> Always explain how your quoted evidence contributes to the work's meaning — not just what it says.
                </p>
              </div>
              ...
              <div style={{ marginTop: 20, padding: "10px 14px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 12, color: "#888" }}>Essay raw total ({frqRaw}/{FRQ_TOTAL_RAW}) contributes (55% weight)</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: "#203567" }}>{frqComponent} / 55</span>
              </div>
          ...
          <div>
            <div style={{ borderRadius: 24, background: scoreBg, border: `2px solid ${scoreColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ textAlign: "left" }}>
                ...
                <BreakdownBar label="Poetry" score={frq[0]} max={FRQ_CONFIG[0].max} color="#4a7cc7" />
                <BreakdownBar label="Prose Fiction" score={frq[1]} max={FRQ_CONFIG[1].max} color="#4a7cc7" />
                <BreakdownBar label="Lit. Argument" score={frq[2]} max={FRQ_CONFIG[2].max} color="#4a7cc7" />
                  ...
                  return (
                    <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12, border: "1.5px solid #eef0f8", background: "white" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#aaa" }}>MCQ {a.mcq}/{MCQ_TOTAL} · Essays {a.frq.reduce((x, y) => x + y, 0)}/{FRQ_TOTAL_RAW}</div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP English Literature score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 540, margin: "0 auto" }}>Essays carry 55% of the composite — more than MCQ — making close reading and literary analysis writing the primary driver of your score.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "45% of total score", body: "55 questions in 1 hour across two literary text sets — one poetry passage and one prose passage. Questions test close reading: understanding tone, imagery, structure, point of view, figurative language, and how literary choices create meaning.", formula: "MCQ / 55 × 45" },
              { Icon: BookMarked, color: "#4a7cc7", title: "Essay Rubric", sub: "6 pts per essay", body: "Each essay scored: Thesis (0–1 pt) — a defensible interpretive claim about the literary text. Evidence & Commentary (0–4 pts) — specific textual evidence with explanation of how it contributes to meaning. Sophistication (0–1 pt) — nuanced, complex literary interpretation.", formula: "thesis + evidence + soph." },
              { Icon: TrendingUp, color: "#2d6e9e", title: "Essays Carry More", sub: "55% of total score", body: "Three essays (poetry analysis, prose fiction analysis, literary argument) in 2 hours. Essay 3 is unique: you choose the literary work. Preparing 4–5 complex works deeply — with specific scenes, symbols, and character analysis — is the highest-leverage exam strategy.", formula: "FRQ_raw / 18 × 55" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥72 → 5 · ≥58 → 4 · ≥44 → 3 · ≥30 → 2 · below → 1. About 8–10% of students earn a 5. Poetry analysis is the essay where most students lose the most points — practicing unfamiliar poem annotation is the single highest-leverage pre-exam skill.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP English Literature — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited AP Lit practice prompts", body: "Generate poetry analysis, prose analysis, and literary argument prompts from your reading list and course texts. Practice every essay type until the analytical structure is automatic." },
              { Icon: Brain, title: "Master close reading and commentary", body: "The evidence & commentary row is worth 4 of 6 essay points. Practice explaining how every quoted line contributes to meaning — tone, theme, character, structure — not just what it says." },
              { Icon: TrendingUp, title: "Track essay scores by type", body: "Log scores for each essay type across practice sessions. Know whether poetry analysis, prose analysis, or the literary argument is your weakest area before exam day." },
              { Icon: Clock, title: "Build literary argument fluency", body: "Essay 3 rewards deep knowledge of a few works. Practice writing about your prepared texts from different angles — symbol, character, structure — so any prompt is answerable." },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><BookMarked size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP English Literature Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP English Literature and Composition is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP English Literature reading list into unlimited close reading practice and timed essay drills. Track your progress to a 5.</p>
