...
// ─── AP Human Geography 2026 Scoring ──────────────────────────────────────
// Section 1: 60 MCQ → 50% of composite · 1h
// Section 2: 3 FRQ → 50% of composite · 1h 15min
//   FRQ 1: /7
//   FRQ 2: /7
//   FRQ 3: /7  (one question includes a data stimulus)
//   Total FRQ raw = 21 pts
// Composite = (MCQ/60 × 50) + (FRQ_raw/21 × 50) → /100
// AP Score thresholds (approximate, College Board 2024–2025 data)
// AP Human Geography has one of the largest test populations (~250k+)
// 5 rate ~12–16%, passing rate ~55–60%
...
const FRQ_CONFIG = [
  {
    label: "FRQ 1",
    max: 7,
    desc: "A multi-part question applying geographic concepts to a specific scenario or data stimulus — typically involves defining a concept, providing an example, and explaining a geographic pattern or process",
  },
  {
    label: "FRQ 2",
    max: 7,
    desc: "A second multi-part question requiring application of a different geographic concept — may involve maps, spatial patterns, or theoretical models (e.g., von Thünen, Burgess, Ravenstein)",
  },
  {
    label: "FRQ 3",
    max: 7,
    desc: "A third multi-part question often focused on a contemporary geographic issue — development, globalization, population policy, or urban change; requires specific evidence and geographic reasoning",
  },
];

const FRQ_TOTAL_RAW = 21;
const MCQ_TOTAL     = 60;
...
function getScoreMessage(score: number): string {
  return ({
    5: "Outstanding — you've mastered geographic concepts, models, and spatial reasoning across all seven units.",
    4: "Strong result. A 4 on AP Human Geography shows genuine command of geographic patterns and their real-world causes.",
    3: "You're in qualifying territory. Sharpen your FRQ examples and geographic vocabulary to push into a 4.",
    2: "You're building the foundation. Focus on population patterns and urban models — the highest-yield concepts on both sections.",
    1: "Don't be discouraged. Consistent daily practice on AP Human Geography concepts and case studies will turn this around.",
...
const DB_NAME = "lunora_ap_hug"; const DB_VERSION = 1; const STORE = "attempts";
...
export default function APHumanGeographyScoreCalculator() {
  const [mcq, setMcq] = useState(36);
  const [frq, setFrq] = useState<number[]>([5, 4, 4]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP Human Geography Score Calculator", "description": "Free AP Human Geography score calculator for 2026. Enter your MCQ and FRQ scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-human-geography-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>60 questions · <strong style={{ color: "#203567" }}>50%</strong> of total score · 1h</p>
                </div>
              </div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>3 questions · 7 pts each · <strong style={{ color: "#203567" }}>50%</strong> of total score · 1h 15min</p>
                </div>
              </div>
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Specific examples earn points — vague answers do not.</strong> Name a real country, city, or event for every example part. "A developing country" earns zero; "Bangladesh" or "sub-Saharan Africa" earns full credit.
                </p>
              </div>
          ...
          <div>
            <div style={{ borderRadius: 24, background: scoreBg, border: `2px solid ${scoreColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ textAlign: "left" }}>
                ...
                {FRQ_CONFIG.map((q, i) => <BreakdownBar key={i} label={`FRQ ${i + 1}`} score={frq[i]} max={q.max} color="#4a7cc7" />)}
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP Human Geography score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 520, margin: "0 auto" }}>MCQ and FRQ carry equal weight — but the FRQ's demand for specific real-world examples makes geographic case knowledge the critical differentiator.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "50% of total score", body: "60 questions in 1 hour covering all seven APHUG units. Questions test geographic vocabulary, model details, and spatial reasoning. Units 2 (population/migration), 6 (urban), and 7 (development) receive the heaviest MCQ emphasis.", formula: "MCQ / 60 × 50" },
              { Icon: Globe, color: "#4a7cc7", title: "Section 2: FRQ", sub: "50% of total score", body: "3 questions in 1 hour 15 minutes, each worth 7 points. Points are awarded per part: define a term, provide a named real-world example, explain a geographic process. At least one FRQ includes a data stimulus (map, graph, or chart).", formula: "FRQ_raw / 21 × 50" },
              { Icon: TrendingUp, color: "#2d6e9e", title: "Specificity Wins", sub: "examples earn points", body: "Every FRQ example part requires a named, specific place — a country, city, region, or event. Vague answers ('a developing country') earn zero. The Demographic Transition Model, von Thünen model, and urban models must be applied to real examples.", formula: "name · explain · apply" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥70 → 5 · ≥55 → 4 · ≥40 → 3 · ≥26 → 2 · below → 1. About 12–16% of students earn a 5. With 250k+ test takers, APHUG is one of the most-taken AP exams — and passing rate (~55–60%) reflects wide preparation levels.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP Human Geography — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited APHUG practice questions", body: "Generate MCQ and FRQ-style questions on every unit — population, migration, urban models, development — directly from your own notes and textbook chapters." },
              { Icon: Brain, title: "Master geographic models and case studies", body: "Struggling with the Demographic Transition Model or von Thünen? Get instant concept breakdowns, model diagrams, and real-world examples for every testable model." },
              { Icon: TrendingUp, title: "Track mastery across all 7 units", body: "See your accuracy unit by unit. Know exactly which APHUG units — cultural patterns, political geography, industrial development — still need work before exam day." },
              { Icon: Clock, title: "Short daily sessions that work", body: "APHUG rewards breadth. Focused daily sessions rotating across all seven units build the comprehensive vocabulary and case knowledge the 60-question MCQ demands." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP Human Geography Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP Human Geography is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP Human Geography notes into unlimited unit-by-unit practice questions. Track your progress to a 5.</p>
