...
import {
  FunctionSquare, BarChart2, Brain, BookOpen,
  ChevronDown, Trash2, History,
  CheckCircle, Sparkles,
  Trophy, Target, TrendingUp, Clock,
  ArrowRight, RotateCcw, Info, Sigma,
} from "lucide-react";

// ─── AP Calculus BC 2026 Scoring ─────────────────────────────────────────────
// Section 1: 45 MCQ → 50% of composite
//   Part A: 30 questions (no calculator, 60 min)
//   Part B: 15 questions (graphing calculator, 45 min)
// Section 2: 6 FRQ → 50% of composite
//   Part A: 2 questions (graphing calculator, 30 min)
//   Part B: 4 questions (no calculator, 60 min)
//   Each FRQ is worth 9 points → total FRQ raw = 54 pts
// Composite = (MCQ/45 × 50) + (FRQ_raw/54 × 50)  → /100
// AP Score thresholds (approximate, College Board 2024–2025 data)
// BC has a higher 5 rate than AB (~40%) because BC students self-select
...
const FRQ_CONFIG = [
  { label: "FRQ 1 (Calculator)",    max: 9, part: "A" },
  { label: "FRQ 2 (Calculator)",    max: 9, part: "A" },
  { label: "FRQ 3 (No Calculator)", max: 9, part: "B" },
  { label: "FRQ 4 (No Calculator)", max: 9, part: "B" },
  { label: "FRQ 5 (No Calculator)", max: 9, part: "B" },
  { label: "FRQ 6 (No Calculator)", max: 9, part: "B" },
];

const FRQ_TOTAL_RAW = 54;
const MCQ_TOTAL     = 45;
...
function getScoreMessage(score: number): string {
  return ({
    5: "Exceptional — you're on track for a 5 on one of the most rigorous AP exams. Series and parametrics are yours.",
    4: "Strong result. A 4 on AP Calc BC earns Calc I and Calc II credit at most universities.",
    3: "You're in qualifying territory. Tighten up your series convergence tests to push into a 4.",
    2: "You're building the foundation. Focus on integration techniques and polar coordinates to move up fast.",
    1: "Don't be discouraged. Consistent daily practice on core calculus concepts will turn this around.",
  } as Record<number, string>)[score] ?? "";
}

function getThresholdRange(score: number): string {
  return ({ 5: "≥70", 4: "57–69", 3: "40–56", 2: "28–39", 1: "0–27" } as Record<number, string>)[score] ?? "";
}
...
const DB_NAME = "lunora_ap_calc_bc"; const DB_VERSION = 1; const STORE = "attempts";
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function APCalcBCScoreCalculator() {
  const [mcq, setMcq]                     = useState(30);
  const [frq, setFrq]                     = useState<number[]>([7, 7, 6, 6, 6, 6]);
  ...
  const faqs = [
    {
      q: "How is the AP Calculus BC exam scored in 2026?",
      a: "The AP Calculus BC exam has two sections. Section 1 is 45 multiple choice questions (Part A: 30 questions with no calculator in 60 minutes; Part B: 15 questions with a graphing calculator in 45 minutes) worth 50% of your total score. Section 2 is 6 free response questions (Part A: 2 questions with a graphing calculator in 30 minutes; Part B: 4 questions without a calculator in 60 minutes) worth the other 50%. Each FRQ is worth 9 points for a total raw FRQ score of 54. Your raw scores convert to a composite out of 100, which maps to an AP score of 1–5.",
    },
    {
      q: "What composite score do I need for a 5 on AP Calculus BC?",
      a: "Based on recent College Board score distributions, you generally need a composite score of approximately 70 or above to earn a 5 on AP Calculus BC. AP Calculus BC has one of the highest 5 rates of all AP exams — roughly 38–43% of test takers score a 5 — because the course self-selects for students with strong math backgrounds.",
    },
    {
      q: "What composite score do I need for a 3 on AP Calculus BC?",
      a: "A composite score of approximately 40 or above typically earns a 3 on AP Calculus BC. A 3 on BC earns college credit equivalent to one semester of calculus at most universities, while a 4 or 5 often earns credit for both Calc I and Calc II.",
    },
    {
      q: "How many free response questions are on the AP Calculus BC exam?",
      a: "The AP Calculus BC free response section has 6 questions total, each worth 9 points for a total raw FRQ score of 54. Part A has 2 questions completed with a graphing calculator in 30 minutes. Part B has 4 questions completed without a calculator in 60 minutes. BC FRQs frequently include series, parametric, and polar questions not found on AP Calculus AB.",
    },
    {
      q: "What is the difference between AP Calculus AB and AP Calculus BC?",
      a: "AP Calculus AB covers roughly one semester of college calculus (Calc I), including limits, derivatives, and basic integration. AP Calculus BC covers Calc I and Calc II material — everything in AB plus additional topics including sequences and series (Taylor and Maclaurin series), parametric equations, polar coordinates, integration by parts, partial fractions, and improper integrals. BC students can also earn an AB subscore on the BC exam.",
    },
    {
      q: "What is the AB subscore on the AP Calculus BC exam?",
      a: "College Board reports an AB subscore for AP Calculus BC students. This subscore is calculated using only the questions on the BC exam that also appear on the AB exam — approximately 60% of the exam. The AB subscore is reported on a 1–5 scale separately from your BC score and can be used by colleges to place you into appropriate math courses even if you don't earn a qualifying BC score.",
    },
    {
      q: "What topics are unique to AP Calculus BC (not on AB)?",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "description": "Free AP Calculus BC score calculator for 2026. Enter your multiple choice and free response scores to instantly calculate your composite score and predicted AP score of 1–5.",
        "url": "https://lunora.app/tools/ap-calculus-bc-score-calculator",
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "How is the AP Calculus BC exam scored?",
            "acceptedAnswer": { "@type": "Answer", "text": "AP Calculus BC has two sections. Section 1 is 45 multiple choice questions worth 50% of your score. Section 2 is 6 free response questions (each worth 9 pts, total 54 raw pts) worth 50%. Raw scores convert to a composite out of 100, then to an AP score of 1–5." }
          },
          {
            "@type": "Question",
            "name": "What composite score do I need for a 5 on AP Calculus BC?",
            "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 70 or above to earn a 5 on AP Calculus BC. About 38–43% of students score a 5, making BC one of the highest 5-rate AP exams." }
          },
          {
            "@type": "Question",
            "name": "What percent is a 5 on AP Calculus BC?",
            "acceptedAnswer": { "@type": "Answer", "text": "Approximately 38–43% of AP Calculus BC test takers score a 5 each year — one of the highest 5 rates of any AP exam, reflecting the self-selected nature of BC students." }
          },
          {
            "@type": "Question",
            "name": "What is the difference between AP Calculus AB and BC?",
            "acceptedAnswer": { "@type": "Answer", "text": "AP Calculus AB covers Calc I material (limits, derivatives, basic integration). AP Calculus BC covers Calc I and Calc II — everything in AB plus sequences and series, parametric equations, polar coordinates, integration by parts, and more. BC students also receive an AB subscore." }
          },
          {
            "@type": "Question",
            "name": "What is the AB subscore on the AP Calculus BC exam?",
            "acceptedAnswer": { "@type": "Answer", "text": "The AB subscore is a separate 1–5 score calculated from the subset of BC exam questions that overlap with the AB curriculum. It represents roughly 60% of the BC exam and is reported alongside your BC score." }
          },
          {
            "@type": "Question",
            "name": "How many free response questions are on AP Calculus BC?",
            "acceptedAnswer": { "@type": "Answer", "text": "AP Calculus BC has 6 free response questions, each worth 9 points, for a total raw FRQ score of 54. Part A has 2 questions with a graphing calculator; Part B has 4 questions without a calculator." }
          },
        ],
      })}} />
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill">
              <Sigma size={11} color="#203567" strokeWidth={2.5} />
              Free Tool · 2026
            </div>
          </div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>45 questions (Part A: 30 no-calc · Part B: 15 with calc) · 50% of total score · 1h 45min</p>
                </div>
              </div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>6 questions · 9 pts each · 50% of total score · 1h 30min</p>
                </div>
              </div>

              {/* Part A FRQ — Calculator */}
              <div style={{ marginTop: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>Part A — Calculator Permitted</span>
                  <span style={{ fontSize: 11, color: "#bbb" }}>2 questions · 30 min</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  {FRQ_CONFIG.filter(q => q.part === "A").map((q, i) => (
                    <ScoreSlider key={i} label={`Question ${i + 1}`} sublabel="9 pts" value={frq[i]} max={q.max} onChange={v => updateFrq(i, v)} color="#4a7cc7" />
                  ))}
                </div>
              </div>

              {/* Part B FRQ — No Calculator */}
              <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid #f0f3fa" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>Part B — No Calculator</span>
                  <span style={{ fontSize: 11, color: "#bbb" }}>4 questions · 60 min</span>
                </div>
                <div className="frq-grid">
                  {FRQ_CONFIG.filter(q => q.part === "B").map((q, i) => (
                    <ScoreSlider key={i + 2} label={`Question ${i + 3}`} sublabel="9 pts" value={frq[i + 2]} max={q.max} onChange={v => updateFrq(i + 2, v)} color="#4a7cc7" />
                  ))}
                </div>
              </div>

              {/* BC-only topics note */}
              <div style={{ marginTop: 18, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>BC-only topics</strong> — Series (Taylor, Maclaurin, convergence tests), parametric equations, polar coordinates, integration by parts, and partial fractions frequently appear in Part B FRQs.
                </p>
              </div>
            ...
            {[
              {
                ...
                body:  "45 multiple choice questions in 1h 45min. Part A: 30 questions, no calculator, 60 min. Part B: 15 questions, graphing calculator, 45 min. No penalty for wrong answers.",
                formula: "MCQ / 45 × 50",
              },
              {
                Icon: BookOpen, color: "#4a7cc7",
                title: "Section 2: FRQ",   sub: "50% of total score",
                body:  "6 questions in 1h 30min, each worth 9 points. Part A: 2 calculator questions (30 min). Part B: 4 no-calculator questions (60 min). Total raw FRQ = 54 points.",
                formula: "FRQ_raw / 54 × 50",
              },
              {
                Icon: Target,   color: "#2d6e9e",
                title: "Composite Score",  sub: "out of 100",
                body:  "MCQ and FRQ components each contribute up to 50 points for a composite out of 100. BC students also receive a separate AB subscore based on the AB-relevant subset of questions.",
                formula: "MCQ_pts + FRQ_pts",
              },
              {
                Icon: Trophy,   color: "#1a7a4a",
                title: "AP Score",         sub: "1–5",
                body:  "Composite ≥70 → 5 · ≥57 → 4 · ≥40 → 3 · ≥28 → 2 · below → 1. About 38–43% of BC students earn a 5 — far higher than most AP exams due to student self-selection.",
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP Calc BC — Free
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles,   title: "Unlimited AP Calc BC questions",  body: "Generate as many practice questions as you need from your own notes, past FRQs, or any study material — covering every BC topic including series and polar." },
              { Icon: Brain,      title: "Deep-dive on BC-only concepts",   body: "Struggling with Taylor series error bounds or parametric derivatives? Get instant flashcards, concept summaries, and mini quizzes on that exact topic." },
              { Icon: TrendingUp, title: "Track mastery per unit",          body: "See your best score, average score, and subtopic breakdown across every quiz attempt. Know exactly which BC topics you've locked in." },
      ...
      <section style={{ padding: "80px 24px", background: "#fff" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
              <div className="tag-pill"><FunctionSquare size={11} strokeWidth={2.5} />More AP Calculators</div>
            </div>
              ...
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <FunctionSquare size={13} color="#203567" strokeWidth={2} />
                  </div>
                  <div>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>
              Everything you need to know about how AP Calculus BC is scored.
            </p>
          </div>
