...
import {
  ...
  ArrowRight, RotateCcw, Info, FlaskConical,
} from "lucide-react";

// ─── AP Physics C: Mechanics 2026 Scoring ─────────────────────────────────
// Section 1: 35 MCQ → 50% of composite · 45 min
//   All single-select questions
// Section 2: 3 FRQ → 50% of composite · 45 min
...
// Physics C Mechanics has a very high 5 rate (~35–40%) due to self-selected
...
const FRQ_CONFIG = [
  {
    ...
    max: 15,
    desc: "Typically covers kinematics, Newton's laws, or energy — often requires calculus-based derivation",
  },
  {
    label: "Free Response 2",
    max: 15,
    desc: "Often involves rotation, angular momentum, or oscillations (SHM); may include graphical analysis",
  },
  {
    label: "Free Response 3",
    max: 15,
    desc: "Commonly tests gravitation, work-energy theorem, or systems with constraints and calculus integration",
  },
];
...
function getScoreMessage(score: number): string {
  return ({
    5: "Outstanding — you're mastering calculus-based mechanics. Newton's laws, rotation, and oscillations are yours.",
    4: "Strong result. A 4 on AP Physics C Mechanics demonstrates real command of calculus-based physics.",
    3: "You're in qualifying territory. Sharpen your rotational dynamics and FRQ derivations to push higher.",
    2: "You're building the foundation. Focus on Newton's laws with calculus and energy methods to move up fast.",
...
function getThresholdRange(score: number): string {
  return ({ 5: "≥70", 4: "55–69", 3: "39–54", 2: "28–38", 1: "0–27" } as Record<number, string>)[score] ?? "";
}

interface SavedAttempt { id: string; mcq: number; frq: number[]; composite: number; apScore: number; savedAt: number; label: string; }
const DB_NAME = "lunora_ap_phys_c_mech"; const DB_VERSION = 1; const STORE = "attempts";
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid #eef0f8" }}>
      ...
      <div style={{ maxHeight: open ? 360 : 0, opacity: open ? 1 : 0, overflow: "hidden", transition: "max-height 0.35s ease, opacity 0.3s" }}>
...
export default function APPhysicsCMechanicsScoreCalculator() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP Physics C: Mechanics Score Calculator", "description": "Free AP Physics C: Mechanics score calculator for 2026. Enter your multiple choice and free response scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-physics-c-mechanics-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [ { "@type": "Question", "name": "How is the AP Physics C: Mechanics exam scored?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: Mechanics has two 45-minute sections. Section 1 is 35 MCQ worth 50% of your score. Section 2 is 3 FRQs (15 pts each, 45 raw total) worth 50%. Raw scores convert to a composite out of 100, then to an AP score of 1–5." } }, { "@type": "Question", "name": "What composite score do I need for a 5 on AP Physics C: Mechanics?", "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 70 or above to earn a 5 on AP Physics C: Mechanics. About 35–40% of students score a 5 each year." } }, { "@type": "Question", "name": "What topics are on AP Physics C: Mechanics?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: Mechanics covers kinematics, Newton's laws, work/energy/power, linear momentum, rotation, oscillations (SHM), and gravitation — all using calculus." } }, { "@type": "Question", "name": "What is the difference between AP Physics C Mechanics and AP Physics 1?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics 1 is algebra-based. AP Physics C: Mechanics covers similar topics but requires calculus — derivatives and integrals are used throughout every unit." } }, { "@type": "Question", "name": "How many free response questions are on AP Physics C Mechanics?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: Mechanics has 3 free response questions, each worth 15 points, for a total raw FRQ score of 45. You have 45 minutes to complete all three." } } ] }) }} />
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>3 questions · 15 pts each · 50% of total score · 45 min</p>
                </div>
              </div>
              ...
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Calculus is required throughout.</strong> FRQ responses must use derivatives and integrals correctly — setting up the calculus earns points even if arithmetic errors appear downstream.
                </p>
              </div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP Physics C: Mechanics score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 520, margin: "0 auto" }}>Physics C: Mechanics is one of the shortest AP exams — just 90 minutes total — but every question is calculus-intensive.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "50% of total score", body: "35 multiple choice questions in 45 minutes without a calculator. Questions involve kinematics, Newton's laws, rotation, SHM, and gravitation — all requiring calculus reasoning.", formula: "MCQ / 35 × 50" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Section 2: FRQ", sub: "50% of total score", body: "3 free response questions in 45 minutes, each worth 15 points. A calculator is permitted. Questions require multi-step calculus derivations, equation setup, and written justification.", formula: "FRQ_raw / 45 × 50" },
              { Icon: FlaskConical, color: "#2d6e9e", title: "Calculus Throughout", sub: "derivatives & integrals", body: "Unlike Physics 1 or 2, calculus is required — not optional. Variable-force problems require integration, rotational inertia requires integral setup, and SHM derivations start from Newton's second law as a differential equation.", formula: "F = ma → d²x/dt²" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥70 → 5 · ≥55 → 4 · ≥39 → 3 · ≥28 → 2 · below → 1. About 35–40% of students earn a 5, one of the highest rates of any AP exam. Cut scores may shift slightly each year.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP Physics C — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited AP Physics C questions", body: "Generate calculus-based mechanics problems on every unit — from kinematics to gravitation — directly from your own notes, textbook chapters, and problem sets." },
              { Icon: Brain, title: "Master the calculus-physics link", body: "Struggling with setting up rotational inertia integrals or solving SHM differential equations? Get instant worked examples and concept breakdowns on that exact topic." },
              { Icon: TrendingUp, title: "Track mastery per unit", body: "See best score, average score, and topic breakdown across every session. Know exactly which Physics C units you've locked in and which need more work." },
              { Icon: Clock, title: "Short daily sessions that work", body: "Built for busy students balancing calculus and physics simultaneously. Focused 15-minute sessions move your composite score measurably forward." },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><FlaskConical size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP Physics C: Mechanics Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP Physics C: Mechanics is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP Physics C: Mechanics notes and problem sets into unlimited calculus-based practice. Track your progress to a 5.</p>
