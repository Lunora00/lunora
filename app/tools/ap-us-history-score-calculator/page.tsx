...
import {
  ...
  ArrowRight, RotateCcw, Info, Landmark,
} from "lucide-react";

// ─── AP US History 2026 Scoring ───────────────────────────────────────────
// Section 1: 55 MCQ + 3 SAQ → 60% of composite
//   Part A: 55 MCQ · 55 min · 40% of composite
//   Part B: 3 SAQ · 40 min · 20% of composite
//     SAQ 1: /3  (required — period 1–7 contextualization/causation)
//     SAQ 2: /3  (required — period 1–7 continuity/change)
//     SAQ 3 OR 4: /3  (student choice — period 1–5 OR period 6–9)
//   Total SAQ raw = 9 pts
...
// Composite formula (approximate conversion to /100):
...
// APUSH has a moderate 5 rate (~13–15%) and a passing rate (~55–60%)
...
const MCQ_TOTAL    = 55;
const SAQ_TOTAL    = 9;   // 3 questions × 3 pts each
const DBQ_MAX      = 7;
const LEQ_MAX      = 6;
...
function getScoreMessage(score: number): string {
  return ({
    5: "Outstanding — you've mastered APUSH argumentation, periodization, and sourcing. An exceptional result on a writing-heavy exam.",
    4: "Strong result. A 4 on AP US History demonstrates real command of historical thinking and essay construction.",
    3: "You're in qualifying territory. Sharpen your DBQ sourcing and LEQ thesis complexity to push into a 4.",
    2: "You're building the foundation. Focus on your DBQ argument structure and SAQ contextualization to move up fast.",
    1: "Don't be discouraged. Consistent daily practice on APUSH historical thinking skills will turn this around.",
...
const DB_NAME = "lunora_ap_apush"; const DB_VERSION = 1; const STORE = "attempts";
...
export default function APUSHistoryScoreCalculator() {
    ...
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .section-divider { height: 1px; background: #f0f3fa; margin: 24px 0; }
      `}</style>

      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP US History Score Calculator", "description": "Free AP US History score calculator for 2026. Enter your MCQ, SAQ, DBQ, and LEQ scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-us-history-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [ { "@type": "Question", "name": "How is the AP US History exam scored?", "acceptedAnswer": { "@type": "Answer", "text": "APUSH has two sections. Section 1 (60% of composite): 55 MCQ worth 40% and 3 SAQs worth 20%. Section 2 (40% of composite): 1 DBQ worth 25% (out of 7) and 1 LEQ worth 15% (out of 6). All scores convert to a composite out of 100, then to an AP score of 1–5." } }, { "@type": "Question", "name": "What composite score do I need for a 5 on AP US History?", "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 72 or above to earn a 5 on AP US History. About 13–15% of students score a 5 each year." } }, { "@type": "Question", "name": "How is the AP US History DBQ scored?", "acceptedAnswer": { "@type": "Answer", "text": "The DBQ is scored out of 7 points: 1 for thesis, 1 for contextualization, up to 3 for evidence and document sourcing (HAPP), and 1 for complexity. It counts for 25% of your composite score." } }, { "@type": "Question", "name": "How is the AP US History LEQ scored?", "acceptedAnswer": { "@type": "Answer", "text": "The LEQ is scored out of 6 points: 1 for thesis, 1 for contextualization, up to 2 for evidence, 1 for historical reasoning skill, and 1 for complexity. It counts for 15% of your composite." } }, { "@type": "Question", "name": "What are SAQs on AP US History?", "acceptedAnswer": { "@type": "Answer", "text": "Short Answer Questions: 3 questions (each worth 3 pts) in 40 minutes, worth 20% of composite. Questions 1 and 2 are required; Question 3 OR 4 is student choice by time period. No thesis required." } } ] }) }} />
      ...
      <section style={{ padding: "0 24px 80px" }}>
        <div className="calc-grid">
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              ...
              <div style={{ marginBottom: 14, padding: "11px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Q1 & Q2 are required.</strong> Q3 OR Q4 is your choice — one covers periods 1–5, the other periods 6–9. Each part (a/b/c) is worth 1 point. No thesis needed; answers must be specific and concise.
                </p>
              </div>
            ...
            <div className="score-card" style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,53,103,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Landmark size={17} color="#203567" strokeWidth={2} /></div>
                <div>
            ...
            <div className="score-card">
              ...
              <div style={{ marginBottom: 16, padding: "11px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Rubric: Thesis (1) · Contextualization (1) · Evidence (2) · Historical Reasoning Skill (1) · Complexity (1).</strong> Choose the prompt where your content knowledge is strongest — all three options use the same rubric.
                </p>
              </div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP US History score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 540, margin: "0 auto" }}>APUSH has four distinct components — MCQ, SAQ, DBQ, and LEQ — each with a different weight and rubric.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "MCQ (40%)", sub: "55 questions · 55 min", body: "Stimulus-based sets of 3–4 questions built around a primary source, secondary source, map, or image. Each set tests historical thinking skills — causation, comparison, continuity and change over time, or periodization.", formula: "MCQ / 55 × 40" },
              { Icon: BookOpen, color: "#4a7cc7", title: "SAQ (20%)", sub: "3 questions · 40 min", body: "Three-part questions requiring specific, concise written responses. No thesis required. Q1 & Q2 are mandatory; Q3 or Q4 is student choice by time period. Each part (a/b/c) earns 1 point.", formula: "SAQ / 9 × 20" },
              { Icon: Landmark, color: "#2d6e9e", title: "DBQ (25%)", sub: "1 essay · 7 pts · 60 min", body: "The highest-value single question on the exam. Requires a thesis, contextualization, use of at least 6 documents, HAPP sourcing on 3+ documents, outside evidence, and demonstration of complexity.", formula: "DBQ / 7 × 25" },
              { Icon: Trophy, color: "#1a7a4a", title: "LEQ (15%)", sub: "1 essay · 6 pts · 40 min", body: "Choose one of three prompts testing comparison, causation, or continuity and change over time. Same rubric structure as the DBQ minus the document requirements. Complexity point requires sophisticated argumentation.", formula: "LEQ / 6 × 15" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP US History — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited APUSH practice questions", body: "Generate MCQ sets, SAQ prompts, and essay outlines on every period — from Period 1 (1491) through Period 9 (1980–present) — from your own notes and textbook chapters." },
              { Icon: Brain, title: "Master DBQ and LEQ argumentation", body: "Struggling with contextualization or complexity? Get instant feedback and worked examples on HAPP sourcing, thesis construction, and complexity strategies for every time period." },
              { Icon: TrendingUp, title: "Track mastery by period", body: "See your accuracy across all nine APUSH periods. Know exactly which eras — Reconstruction, the Progressive Era, the Cold War — you've locked in and which need more work." },
              { Icon: Clock, title: "Short daily sessions that work", body: "APUSH rewards breadth across nine periods and depth on essay skills. Focused daily sessions that mix content review with essay practice build both at once." },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Landmark size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP US History Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP US History is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your APUSH notes into unlimited period-by-period practice questions and essay prompts. Track your progress to a 5.</p>
