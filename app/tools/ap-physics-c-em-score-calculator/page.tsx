...
import {
  ...
  ArrowRight, RotateCcw, Info, Waves,
} from "lucide-react";

// ─── AP Physics C: Electricity & Magnetism 2026 Scoring ───────────────────
// Section 1: 35 MCQ → 50% of composite · 45 min · no calculator
// Section 2: 3 FRQ → 50% of composite · 45 min · calculator permitted
//   Each FRQ is worth 15 points → total FRQ raw = 45 pts
// Composite = (MCQ/35 × 50) + (FRQ_raw/45 × 50) → /100
// AP Score thresholds (approximate, College Board 2024–2025 data)
// Physics C: E&M has a high 5 rate (~30–35%) but is considered the hardest
// AP exam due to the density and abstraction of Gauss's law, Ampere's law,
// Faraday's law, and circuit analysis with calculus
...
const FRQ_CONFIG = [
  {
    label: "Free Response 1",
    max: 15,
    desc: "Typically electrostatics — Gauss's law, electric field/potential derivations, or capacitor analysis with calculus",
  },
  {
    label: "Free Response 2",
    max: 15,
    desc: "Often magnetism or electromagnetic induction — Ampere's law, Faraday's law, or circuits with inductors/capacitors",
  },
  {
    label: "Free Response 3",
    max: 15,
    desc: "Commonly RC/RL/LC circuits with differential equations, energy methods, or combined E&M scenarios",
  },
];

const FRQ_TOTAL_RAW = 45;
const MCQ_TOTAL     = 35;
...
function getScoreMessage(score: number): string {
  return ({
    5: "Exceptional — you've mastered calculus-based E&M. Gauss's law, Faraday's law, and circuit analysis are yours.",
    4: "Strong result. A 4 on AP Physics C: E&M is a genuine achievement in one of the hardest AP exams.",
    3: "You're in qualifying territory. Tighten up your Ampere's law applications and RC/RL circuit derivations to push higher.",
    2: "You're building the foundation. Focus on electric field and potential derivations using Gauss's law to move up fast.",
    1: "Don't be discouraged. Consistent daily practice on core E&M concepts will turn this around.",
  } as Record<number, string>)[score] ?? "";
}
function getThresholdRange(score: number): string {
  return ({ 5: "≥70", 4: "55–69", 3: "38–54", 2: "26–37", 1: "0–25" } as Record<number, string>)[score] ?? "";
}

interface SavedAttempt { id: string; mcq: number; frq: number[]; composite: number; apScore: number; savedAt: number; label: string; }
const DB_NAME = "lunora_ap_phys_c_em"; const DB_VERSION = 1; const STORE = "attempts";
...
export default function APPhysicsCEMScoreCalculator() {
  const [mcq, setMcq] = useState(22);
  const [frq, setFrq] = useState<number[]>([10, 10, 10]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "AP Physics C: Electricity & Magnetism Score Calculator", "description": "Free AP Physics C: E&M score calculator for 2026. Enter your multiple choice and free response scores to instantly calculate your composite score and predicted AP score of 1–5.", "url": "https://lunora.app/tools/ap-physics-c-em-score-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [ { "@type": "Question", "name": "How is the AP Physics C: E&M exam scored?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: E&M has two 45-minute sections. Section 1 is 35 MCQ (no calculator) worth 50%. Section 2 is 3 FRQs (15 pts each, 45 raw total, calculator permitted) worth 50%. Raw scores convert to a composite out of 100, then to an AP score of 1–5." } }, { "@type": "Question", "name": "What composite score do I need for a 5 on AP Physics C: E&M?", "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 70 or above to earn a 5 on AP Physics C: E&M. About 30–35% of students score a 5 each year." } }, { "@type": "Question", "name": "What topics are on AP Physics C: E&M?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: E&M covers electrostatics (Gauss's law), conductors and capacitors, DC circuits (RC circuits with differential equations), magnetic fields (Biot-Savart law, Ampere's law), electromagnetic induction (Faraday's law, RL circuits), and Maxwell's equations in integral form." } }, { "@type": "Question", "name": "Is AP Physics C: E&M harder than AP Physics C: Mechanics?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: E&M is generally considered harder than Mechanics due to greater mathematical abstraction — Gauss's law, Ampere's law, and Faraday's law require surface and line integrals, and circuit problems require solving differential equations." } }, { "@type": "Question", "name": "How many free response questions are on AP Physics C: E&M?", "acceptedAnswer": { "@type": "Answer", "text": "AP Physics C: E&M has 3 free response questions, each worth 15 points, for a total raw FRQ score of 45. You have 45 minutes and a calculator is permitted." } } ] }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: "clamp(28px,5vw,54px)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.03em", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>
            AP Physics C: E&M{" "}
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>35 questions · 50% of total score · 45 min · calculator not permitted</p>
                </div>
              </div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>3 questions · 15 pts each · 50% of total score · 45 min · calculator permitted</p>
                </div>
              </div>
              ...
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Integral setup earns points.</strong> Correctly identifying your Gaussian or Amperian surface and writing the integral form of the law earns partial credit — even if later arithmetic contains errors.
                </p>
              </div>
                ...
                {FRQ_CONFIG.map((q, i) => (
                  <div key={i}>
                    <div style={{ marginBottom: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#203567" }}>Question {i + 1} — {q.label}</span>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP Physics C: E&M score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 520, margin: "0 auto" }}>90 minutes total, split evenly between MCQ and FRQ — but every point demands calculus fluency and conceptual precision.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "50% of total score", body: "35 multiple choice questions in 45 minutes without a calculator. Questions span electrostatics, circuits, magnetism, and induction — all requiring calculus reasoning and integral-form law application.", formula: "MCQ / 35 × 50" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Section 2: FRQ", sub: "50% of total score", body: "3 free response questions in 45 minutes, each worth 15 points. A calculator is permitted. Questions require Gaussian/Amperian surface setup, differential equation writing and solving, and multi-step derivations.", formula: "FRQ_raw / 45 × 50" },
              { Icon: Waves, color: "#2d6e9e", title: "Maxwell's Laws in Integral Form", sub: "Gauss · Ampere · Faraday", body: "Unlike Physics 2, every major law is applied in integral form. You must set up surface integrals for flux, line integrals for circulation, and know when each law applies to a given geometry.", formula: "∮E·dA = Q/ε₀" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥70 → 5 · ≥55 → 4 · ≥38 → 3 · ≥26 → 2 · below → 1. About 30–35% of students earn a 5. E&M is widely considered the hardest AP exam. Cut scores may shift slightly each year.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              AP Physics C: E&M rewards students who can set up the right integral before anything else. Most students lose points not because they can't do the math — but because they apply the wrong law, choose the wrong surface, or can't write the circuit differential equation from scratch under time pressure. Upload your E&M notes, textbook chapters, and lab write-ups into Lunora to get unlimited calculus-based E&M practice that drills Gauss's law, Faraday's law, Ampere's law, and RC/RL/LC circuit analysis until the setups are automatic.
            </p>
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP Physics C: E&M — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited AP Physics C: E&M questions", body: "Generate calculus-based E&M problems on every topic — from Gauss's law derivations to Faraday's law induction — directly from your own notes and problem sets." },
              { Icon: Brain, title: "Master integral-form laws", body: "Struggling to choose the right Gaussian surface for a cylindrical charge distribution? Get instant worked examples and surface-selection breakdowns for every geometry." },
              { Icon: TrendingUp, title: "Track mastery per unit", body: "See your best score, average score, and topic breakdown across every session. Know exactly which E&M units — electrostatics, circuits, magnetism, induction — you've locked in." },
              { Icon: Clock, title: "Short daily sessions that work", body: "Built for students simultaneously managing AP Calculus BC and AP Physics C. Focused 15-minute sessions move your composite score measurably forward." },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Waves size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP Physics C: E&M Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP Physics C: Electricity & Magnetism is scored.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP Physics C: E&M notes and problem sets into unlimited calculus-based practice. Track your progress to a 5.</p>
