...
// ─── Indiana University GPA Calculation ──────────────────────────────────────
// Indiana University Bloomington uses a standard 4.0 scale
// A+ = 4.0, A = 4.0, A- = 3.7
// B+ = 3.3, B = 3.0, B- = 2.7
// C+ = 2.3, C = 2.0, C- = 1.7
// D+ = 1.3, D = 1.0, D- = 0.7
// F  = 0.0
// P/F (Pass/Fail), W (withdrawal), I (incomplete), R (deferred) = not counted
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Dean's List: semester GPA ≥ 3.5 with 12+ graded credit hours
// Academic Distinction: cumulative GPA 3.5–3.799 at graduation
// Academic High Distinction: cumulative GPA 3.8–3.999 at graduation
// Academic Highest Distinction: cumulative GPA 4.0 at graduation
...
const GRADE_POINTS: Record<string, number | null> = {
  ...
  "D+": 1.3, "D": 1.0, "D-": 0.7,
  "F":  0.0,
  "P": null, "F*": null, "W": null, "I": null, "R": null,
};

const GRADE_OPTIONS = ["A+","A","A-","B+","B","B-","C+","C","C-","D+","D","D-","F","P","W","I","R"];

function getGPALabel(gpa: number): string {
  if (gpa >= 4.0) return "Highest Distinction";
  if (gpa >= 3.8) return "High Distinction";
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 4.0) return "Perfect — Academic Highest Distinction at Indiana University. A 4.0 cumulative GPA is the pinnacle of academic achievement at IU Bloomington. Exceptional work.";
  if (gpa >= 3.8) return "Academic High Distinction territory at IU. You're performing at an elite level — this GPA opens doors to the most competitive graduate programs and scholarships.";
  if (gpa >= 3.5) return "Dean's List standing at Indiana University. This reflects genuine academic excellence and qualifies you for merit scholarships and honors recognition at IU.";
  if (gpa >= 3.0) return "Solid academic standing at IU Bloomington. Push toward 3.5 to earn Dean's List recognition and strengthen your graduate school and career prospects.";
  if (gpa >= 2.0) return "You're meeting IU's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use IU's Student Academic Center and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit IU's Academic Support Center and connect with your school advisor as soon as possible.";
}
...
const DB_NAME = "lunora_iu_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. ECON E201)`} value={course.name}
...
export default function IUGPACalculator() {
  ...
  const faqs = [
    {
      q: "How is GPA calculated at Indiana University Bloomington?",
      a: "Indiana University Bloomington calculates GPA by dividing total grade points earned by total credit hours attempted with a letter grade. For each course you multiply the grade point value by credit hours — A/A+ = 4.0, A- = 3.7, B+ = 3.3, B = 3.0, and so on down to F = 0.0. Pass/Fail (P/F), W (withdrawal), I (incomplete), and R (deferred grade) are excluded entirely from your GPA. IU uses a full plus/minus scale including D+ (1.3), D (1.0), and D- (0.7).",
    },
    {
      q: "What GPA do I need for Dean's List at IU Bloomington?",
      a: "To earn Dean's List recognition at Indiana University Bloomington, you need a semester GPA of 3.5 or higher while completing at least 12 graded credit hours in that term. Dean's List is awarded per semester — a strong single semester qualifies you regardless of your cumulative GPA. The distinction appears on your academic transcript and is recognized by IU's individual schools including the Kelley School of Business and the Luddy School of Informatics.",
    },
    {
      q: "What cumulative GPA do I need for graduation honors at IU?",
      a: "Indiana University awards three tiers of graduation honors based on cumulative GPA. Academic Distinction requires a cumulative GPA of 3.50 to 3.79. Academic High Distinction is awarded for a GPA of 3.80 to 3.99. Academic Highest Distinction — the top honor at IU — requires a perfect cumulative GPA of 4.0. You must complete a minimum number of credit hours at IU to be eligible for any graduation honor tier.",
    },
    {
      q: "What is the minimum GPA to stay in good standing at Indiana University?",
      a: "Indiana University requires a minimum cumulative GPA of 2.0 to remain in good academic standing. Students who fall below 2.0 are placed on academic probation and must raise their GPA to avoid academic dismissal. Several IU schools and programs — including the Kelley School of Business, the Jacobs School of Music, and the O'Neill School of Public and Environmental Affairs — maintain higher continuation GPA requirements for their majors.",
    },
    {
      q: "Does IU offer grade replacement or forgiveness?",
      a: "Indiana University offers a Course Repeat Policy that allows undergraduate students to retake a course and, under certain conditions, have the better grade used in GPA calculations while both attempts remain on the transcript. The specifics vary by school — Kelley, College of Arts and Sciences, and other IU schools have slightly different rules. There are limits on how many times you can repeat a course and how many total courses can be forgiven. Check with your academic advisor or the IU Registrar for current policies in your school.",
    },
    {
      q: "How does IU's Pass/Fail grading option affect my GPA?",
      a: "Courses taken on a Pass/Fail basis at Indiana University are completely excluded from GPA calculations. A P grade earns credit but contributes no grade points; an F under Pass/Fail (sometimes noted separately) may or may not count depending on specific school policies — check with your advisor. Most IU major requirements must be completed for a letter grade. Using Pass/Fail strategically for electives or exploration courses can help protect your GPA while you develop broader interests.",
    },
    {
      q: "What does a W (withdrawal) do to my GPA at IU?",
      a: "A W grade at Indiana University does not affect your GPA — it notes the withdrawal from the course without assigning grade points. However, accumulated W grades can raise flags with graduate school admissions, professional school applications, and financial aid offices. IU has specific deadlines for dropping a course with a W versus receiving a grade, so always check the academic calendar. If you're considering withdrawing, speak with your advisor first.",
    },
    {
      q: "How do I calculate my cumulative GPA across multiple semesters at IU?",
      a: "To calculate your IU cumulative GPA, add up the grade-point products (grade points × credit hours) from every letter-graded semester across your time at Indiana University, then divide by total letter-graded credit hours attempted. The Cumulative GPA mode in this calculator makes it simple: enter your current GPA and total credit hours from previous semesters, add this semester's courses, and your updated cumulative GPA appears instantly.",
    },
    {
      q: "What GPA do I need to get into the Kelley School of Business at IU?",
      a: "The Kelley School of Business at Indiana University is one of the top-ranked undergraduate business programs in the United States. Internal admission to Kelley after your freshman year is competitive — typically requiring a cumulative GPA of 3.0 or above, along with completion of specific prerequisite courses like accounting, calculus, and economics. Many competitive admits have GPAs of 3.3 to 3.7 or higher. Check Kelley's current admission criteria directly, as requirements can change by year.",
    },
    {
      q: "How can I raise my GPA at Indiana University?",
      a: "Raising your GPA at IU is most impactful early in your academic career when your credit hour total is smaller. The highest-leverage strategies are: retaking courses where you earned a D or F under IU's repeat policy, using Pass/Fail strategically for exploratory electives, targeting high-credit courses where you can excel, and replacing passive review with active recall practice. Lunora lets you generate targeted practice questions from your actual IU course materials — from ECON E201 to BIOL L211 to BUS M300 — so you walk into every exam knowing exactly where your knowledge gaps are.",
    },
  ];
  ...
  const honorThresholds = [
    { label: "Highest Distinction", range: "4.00",        color: "#1a6e3c" },
    { label: "High Distinction",    range: "3.80 – 3.99", color: "#1a6e3c" },
    { label: "Distinction / Dean's",range: "3.50 – 3.79", color: "#1e5fa8" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "IU GPA Calculator", "description": "Free Indiana University GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your IU Bloomington semester GPA and cumulative GPA. Supports IU's full grading scale including A+, D-, P/F, W, and R grades.", "url": "https://lunora.app/tools/gpa-calculator/iu-gpa-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · Indiana University Bloomington</div>
          </div>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 620, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Indiana University Bloomington</strong> semester GPA and cumulative GPA instantly. Built for IU's exact grading scale — A+ through D−, P/F, W, I, and R exclusions all handled correctly.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "Full IU 4.0 scale with D−" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your IU Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>P/F, W, I, and R grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at IU</h2>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>IU Honor Thresholds</span>
                </div>
                {honorThresholds.map(({ label, range, color }) => {
                  const isActive = getGPALabel(displayGPA) === label ||
                    (label === "Distinction / Dean's" && displayGPA >= 3.5 && displayGPA < 3.8);
                  return (
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />IU Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How Indiana University GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 580, margin: "0 auto" }}>IU uses a full 4.0 plus/minus scale including D− (0.7). P/F, W, I, and R grades are excluded entirely from your GPA calculation.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "IU Grade Scale", sub: "4.0 max · Full plus/minus", body: "A+/A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · F = 0.0. P, W, I, and R are excluded.", formula: "grade_pts × credit_hrs" },
              ...
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at IU, then divide by total graded credit hours attempted across your entire academic record.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#1a6e3c", title: "IU Honors", sub: "Graduation distinctions", body: "4.0 → Highest Distinction · 3.80–3.99 → High Distinction · 3.50–3.79 → Distinction · 3.50+ semester → Dean's List (12+ graded hrs) · Below 2.0 → probation.", formula: "cumulative GPA → honor" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              IU Bloomington is a Big Ten research university with serious academic expectations — especially in Kelley, Luddy, and the sciences. Passive studying creates the feeling of preparation without the results. Lunora lets you upload your actual IU course materials and generates unlimited active recall questions on exactly the content your professor will test. Whether you're chasing Dean's List or recovering from a tough semester, daily targeted practice is the highest-ROI study habit you can build at Indiana.
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own IU notes", body: "Upload your lecture slides, syllabi, and readings from any IU course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From ECON E201 to BIOL L211 to BUS M300 — Lunora adapts to any Indiana University course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every IU exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, built from your actual IU Bloomington course material, compounds dramatically over a full semester. Early investment, outsized GPA return." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>IU GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA works at Indiana University Bloomington.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your IU course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
