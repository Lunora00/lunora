...
// ─── ASU GPA Calculation ─────────────────────────────────────────────────────
// Arizona State University uses a standard 4.0 scale
// A+ = 4.0, A = 4.0, A- = 3.7
// B+ = 3.3, B = 3.0, B- = 2.7
// C+ = 2.3, C = 2.0, C- = 1.7
// D+ = 1.3, D = 1.0, D- = 0.7 (note: ASU does grant D- in some colleges)
// E (fail) = 0.0  (ASU uses "E" not "F" for failure)
// W (withdrawal), I (incomplete), Y, Z = not counted
// P/NP = not counted toward GPA
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Dean's List: 3.5+ with 12+ graded hours; President's List: 4.0
...
const GRADE_OPTIONS = ["A+","A","A-","B+","B","B-","C+","C","C-","D+","D","D-","E","P","NP","W","I"];
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 4.0) return "Perfect. You've earned ASU's President's List — the highest academic distinction Arizona State awards. Exceptional work.";
  if (gpa >= 3.7) return "Outstanding — Summa Cum Laude standing at ASU. This opens doors to the most competitive graduate programs and scholarships.";
  if (gpa >= 3.5) return "Dean's List territory. This GPA reflects real academic excellence and qualifies you for merit scholarships and honors programs.";
  if (gpa >= 3.0) return "Solid academic standing at ASU. Push toward 3.5 to earn Dean's List recognition and unlock scholarship opportunities.";
  if (gpa >= 2.0) return "You're meeting ASU's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use ASU's tutoring centers and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit ASU's Academic Success Center and connect with your college advisor as soon as possible.";
}
...
const DB_NAME = "lunora_asu_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. ENG 101)`} value={course.name}
...
export default function ASUCumulativeGPACalculator() {
  ...
  const faqs = [
    ...
    {
      q: "What cumulative GPA do I need for graduation honors at ASU?",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "ASU Cumulative GPA Calculator", "description": "Free Arizona State University cumulative GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your ASU semester GPA and cumulative GPA across all semesters.", "url": "https://lunora.app/tools/gpa-calculator/cumulative-gpa-calculator-asu", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · Arizona State University</div>
          </div>
          <h1 style={{ fontSize: "clamp(30px,5vw,56px)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.03em", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>
            ASU{" "}
            <span style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", color: "#203567" }}>Cumulative GPA Calculator</span>
          </h1>
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 600, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Arizona State University</strong> semester GPA and cumulative GPA instantly. Supports ASU's grading scale including the E grade and W/I exclusions.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "ASU 4.0 scale with E grade" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
              <input type="text" placeholder="Semester (e.g. Spring 2026)" value={semesterLabel} onChange={e => setSemesterLabel(e.target.value)}
                style={{ padding: "8px 16px", border: "1.5px solid #eef0f8", borderRadius: 100, fontSize: 13, fontWeight: 600, color: "#203567", fontFamily: "'DM Sans', sans-serif", outline: "none", background: "white", minWidth: 200 }} />
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your ASU Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>W, I, P, and NP grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at ASU</h2>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>ASU Honor Thresholds</span>
                </div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />ASU Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How ASU cumulative GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 560, margin: "0 auto" }}>ASU uses a 4.0 scale with one important difference — a failing grade is recorded as E, not F. W and I grades are excluded from GPA calculations.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "ASU Grade Scale", sub: "4.0 maximum · E for fail", body: "A+/A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · E = 0.0. W, I, P, NP are excluded.", formula: "grade_pts × credit_hours" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Semester GPA", sub: "Single semester", body: "For each letter-graded course, multiply the grade point value by credit hours. Sum those products, then divide by total graded credit hours attempted that semester.", formula: "Σ(pts × cr) / Σcr" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at ASU, then divide by total graded credit hours attempted across your entire academic record. This is the GPA on your transcript.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#1a6e3c", title: "ASU Honors", sub: "Graduation distinctions", body: "4.0 → President's List · 3.90+ → Summa Cum Laude · 3.70–3.89 → Magna Cum Laude · 3.50–3.69 → Cum Laude · 3.50+ semester → Dean's List · Below 2.0 → probation.", formula: "cumulative GPA → honor" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              ASU is a massive research university where it's easy to drift behind without realizing it. Passive studying — rereading notes, watching lecture recordings at 2x — creates the feeling of preparation without the results. Lunora flips that by letting you upload your actual ASU course materials and generating unlimited active recall questions on exactly the content your professor will test. Whether you're grinding toward 3.5 for Dean's List or recovering from a rough semester, daily targeted practice is the highest-ROI study habit you can build.
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own ASU notes", body: "Upload your lecture slides, syllabi, and readings from any ASU course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From BIO 181 to MAT 270 to ENG 101 — Lunora adapts to any ASU course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every ASU exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, building from your actual course material, compounds dramatically over a full ASU semester. Early investment, outsized GPA return." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>ASU Cumulative GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how cumulative GPA works at Arizona State University.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your ASU course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
