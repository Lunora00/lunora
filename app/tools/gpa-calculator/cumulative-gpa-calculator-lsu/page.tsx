...
// Purdue University West Lafayette uses a standard 4.0 scale
// A+ = 4.0, A = 4.0, A- = 3.7
// B+ = 3.3, B = 3.0, B- = 2.7
// C+ = 2.3, C = 2.0, C- = 1.7
// D+ = 1.3, D = 1.0, D- = 0.7 (Note: D- technically passing at Purdue but not all programs accept it)
// F = 0.0
// S/U (Satisfactory/Unsatisfactory), W (Withdrawal), I (Incomplete) = not counted toward GPA
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Academic standing:
//   Good standing ≥ 2.0
//   Dean's List: ≥ 3.5 (semester, 12+ credit hours)
//   Semester Honors: 3.5–3.99
//   High Distinction (graduation): ≥ 3.9
//   Distinction (graduation): 3.5–3.89
//   Some colleges (Engineering, Science) require higher minimums for majors
...
const GRADE_POINTS: Record<string, number | null> = {
  ...
  "C+": 2.3, "C": 2.0, "C-": 1.7,
  "D+": 1.3, "D": 1.0, "D-": 0.7,
  "F": 0.0, "S": null, "U": null, "W": null,
};

const GRADE_OPTIONS = ["A+","A","A-","B+","B","B-","C+","C","C-","D+","D","D-","F","S","U","W"];

function getGPALabel(gpa: number): string {
  if (gpa >= 3.9) return "High Distinction";
...
function getGPABg(gpa: number): string {
  if (gpa >= 3.9) return "rgba(26,110,60,0.07)";
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 3.9) return "Outstanding — you're on track for High Distinction honors at Purdue. Elite academic performance.";
  if (gpa >= 3.5) return "Excellent work. Distinction standing and Dean's List eligibility reflect real mastery of your coursework.";
  if (gpa >= 3.0) return "Solid academic standing at Purdue. Push toward 3.5 to qualify for Distinction and Dean's List recognition.";
  if (gpa >= 2.0) return "Meeting Purdue's minimum standards. Focus on your lowest-scoring courses to build a stronger transcript.";
  if (gpa >= 1.0) return "Academic warning territory. Take advantage of Purdue's tutoring and advising resources immediately.";
  return "Immediate action needed. Visit Purdue's Academic Recovery Program and speak with your advisor today.";
}
...
function CourseRow({
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 110px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input
        type="text"
        placeholder={`Course ${index + 1} (e.g. MA 261)`}
        value={course.name}
...
export default function PurdueGPACalculator() {
  ...
  const faqs = [
    {
      ...
      a: "Purdue University West Lafayette uses a standard 4.0 scale. Your GPA is calculated by multiplying each course's grade points by its credit hours, summing all those products, then dividing by your total credit hours attempted. A+ and A both equal 4.0, A- equals 3.7, B+ equals 3.3, B equals 3.0, down to D- at 0.7 and F at 0.0. Satisfactory/Unsatisfactory (S/U) grades and Withdrawals (W) are excluded from GPA calculations.",
    },
    {
      q: "What GPA do I need for the Dean's List at Purdue?",
      a: "To qualify for the Dean's List at Purdue University, you generally need a semester GPA of 3.5 or higher while carrying a full-time load of at least 12 credit hours. Individual colleges within Purdue — such as the College of Engineering or the Daniels School of Business — may have additional criteria. The Dean's List is a per-semester recognition, so it does not require a high cumulative GPA.",
    },
    {
      q: "What GPA is required for graduation honors at Purdue?",
      a: "Purdue awards graduation honors based on your cumulative GPA. Distinction is awarded for a cumulative GPA of 3.5 to 3.89. High Distinction — the highest graduation honor — is awarded for a cumulative GPA of 3.9 or higher. These designations appear on your diploma and official Purdue transcript and are calculated from all letter-graded coursework completed at Purdue.",
    },
    {
      q: "What is the minimum GPA to stay in good standing at Purdue?",
      a: "Purdue requires a minimum cumulative GPA of 2.0 to remain in good academic standing. Students who fall below 2.0 may be placed on academic probation. Many individual programs have higher minimum GPA requirements — notably, Purdue's College of Engineering requires a 2.0 in all major coursework, while some departments require higher minimums for degree completion. Graduate programs at Purdue typically require a 3.0 minimum.",
    },
    {
      q: "How do I calculate my cumulative GPA at Purdue across multiple semesters?",
      a: "To calculate your cumulative GPA at Purdue, you need the total grade points earned and total credit hours attempted across all semesters. Multiply grade points by credit hours for every letter-graded course, sum those products across all semesters, and divide by total credit hours. This calculator's Cumulative GPA mode lets you enter your existing GPA and credit hours to see exactly how your current semester is changing your overall standing.",
    },
    {
      q: "Do S/U (Satisfactory/Unsatisfactory) grades count toward my Purdue GPA?",
      a: "No — Satisfactory (S) and Unsatisfactory (U) grades do not count toward your GPA at Purdue. These grades are excluded from GPA calculations entirely, though they may count toward degree credit hours. Similarly, Withdrawal (W) grades are not included in GPA calculations. This calculator automatically excludes S, U, and W grades when computing your semester and cumulative GPA.",
    },
    {
      q: "What are typical credit hours for Purdue courses?",
      a: "Most lecture courses at Purdue carry 3 credit hours. Laboratory and studio courses typically carry 1–2 credit hours. Some engineering and science lecture courses are 4 credit hours. Seminar and 1-credit courses also exist. Full-time enrollment at Purdue is defined as 12 or more credit hours per semester. Engineering students commonly carry 15–18 credit hours per semester to complete degree requirements on time.",
    },
    {
      q: "How can I raise my Purdue GPA?",
      a: "The fastest way to raise your Purdue GPA is to maximize performance in high-credit-hour courses, since those moves the GPA needle most. Early in your degree (under 60 credits completed), one strong semester can meaningfully shift your cumulative GPA. Later on, you need sustained improvement because the larger credit base dilutes each semester's impact. Grade replacement policies at Purdue (check with your registrar for current rules) may allow you to retake courses where you earned a D or F. The most effective daily habit is active recall practice — uploading your actual Purdue course notes and materials into Lunora to generate targeted practice questions by topic, so every exam session is deliberate rather than passive rereading.",
    },
  ];
  ...
  const apTools = [
    { name: "AP Environmental Science Score Calculator", href: "/tools/ap-score-calculator/ap-environmental-science-score-calculator" },
    { name: "AP Biology Score Calculator",               href: "/tools/ap-score-calculator/ap-biology-score-calculator" },
    { name: "AP Chemistry Score Calculator",             href: "/tools/ap-score-calculator/ap-chemistry-score-calculator" },
    { name: "AP Physics 1 Score Calculator",             href: "/tools/ap-score-calculator/ap-physics-1-score-calculator" },
    { name: "AP Physics 2 Score Calculator",             href: "/tools/ap-score-calculator/ap-physics-2-score-calculator" },
    { name: "AP Calculus AB Score Calculator",           href: "/tools/ap-score-calculator/ap-calculus-ab-score-calculator" },
    { name: "AP Calculus BC Score Calculator",           href: "/tools/ap-score-calculator/ap-calculus-bc-score-calculator" },
    { name: "AP Statistics Score Calculator",            href: "/tools/ap-score-calculator/ap-statistics-score-calculator" },
    { name: "AP US History Score Calculator",            href: "/tools/ap-score-calculator/ap-us-history-score-calculator" },
    { name: "AP Macroeconomics Score Calculator",        href: "/tools/ap-score-calculator/ap-macroeconomics-score-calculator" },
    { name: "AP English Language Score Calculator",      href: "/tools/ap-score-calculator/ap-english-language-score-calculator" },
    { name: "AP Human Geography Score Calculator",       href: "/tools/ap-score-calculator/ap-human-geography-score-calculator" },
    { name: "AP Physics C Mechanics Score Calculator",   href: "/tools/ap-score-calculator/ap-physics-c-mechanics-score-calculator" },
  ];

  const honorThresholds = [
    { label: "High Distinction",           range: "3.90 – 4.00", color: "#1a6e3c" },
    { label: "Distinction / Dean's List",  range: "3.50 – 3.89", color: "#1e5fa8" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "WebApplication",
        "name": "Purdue University GPA Calculator",
        "description": "Free Purdue University GPA calculator for 2026. Enter your courses, credit hours, and grades to instantly calculate your semester GPA and cumulative GPA at Purdue West Lafayette.",
        "url": "https://lunora.app/tools/gpa-calculator/purdue-gpa-calculator",
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · Purdue</div>
          </div>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 600, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Purdue University West Lafayette</strong> semester GPA and cumulative GPA instantly. Enter your courses, credit hours, and grades — including S/U and W grades.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "Purdue 4.0 scale" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>S, U, and W grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
              ...
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>Enter your existing GPA and credit hours to calculate your Purdue cumulative GPA</p>
                  </div>
                </div>
                ...
                {courses.map((c, i) => {
                  ...
                  return (
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                      ...
                      <span style={{ fontSize: 11, color: "#aaa", width: 38, textAlign: "right", flexShrink: 0 }}>{counted ? `${c.credits}cr` : c.grade}</span>
                    </div>
                  );
                })}
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How Purdue GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 540, margin: "0 auto" }}>Purdue uses a standard 4.0 scale. Grade points are weighted by credit hours and divided by total letter-graded credits attempted.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Grade Point Scale", sub: "4.0 maximum", body: "A+/A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · F = 0.0. S, U, and W are excluded.", formula: "grade × credit_hours" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Semester GPA", sub: "Per-semester calculation", body: "Multiply each course's grade points by its credit hours. Sum all those products across letter-graded courses only. Divide by total letter-graded credit hours attempted.", formula: "Σ(pts × credits) / Σcredits" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "Across all semesters", body: "Sum grade-point products and credit hours from every semester on your Purdue record. Your cumulative GPA is the weighted average of all letter-graded coursework.", formula: "(prev_pts + new_pts) / total" },
              { Icon: Trophy, color: "#1a6e3c", title: "Purdue Honor Thresholds", sub: "Graduation distinctions", body: "3.90+ → High Distinction · 3.50–3.89 → Distinction · 3.50+ per semester → Dean's List (12+ credit hours) · 2.0 minimum for good standing · Below 2.0 → probation risk.", formula: "cumulative GPA → honors" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              Purdue is one of the most academically rigorous universities in the country. Engineering students face weed-out sequences like MA 261, PHYS 172, and CHE 205. Even outside engineering, competition in CS, Krannert, and the sciences is fierce. Most students grind through problem sets and reread lecture slides — and then blank on exams because passive study doesn't build the retrieval pathways you need. Lunora lets you upload your actual Purdue lecture notes, syllabi, and textbook chapters to generate unlimited targeted practice questions by topic. You drill exactly what your professor will test, not generic content. Whether you're pushing for a 3.9 or fighting to stay above a 2.0, deliberate daily practice with your own material is the highest-leverage habit you can build.
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              { Icon: Brain, title: "Works for every Purdue college", body: "From MA 261 and ECE 270 to MGT 200 and BIOL 230 — Lunora adapts to any subject. Stop passive rereading. Start active recall, the single most evidence-backed study technique." },
              { Icon: TrendingUp, title: "Track mastery by topic across every course", body: "See your accuracy by topic across all your courses. Know exactly which concepts need another pass before your next exam — never walk in underprepared again." },
              { Icon: Clock, title: "Short daily sessions that compound all semester", body: "Even 15–20 minutes of targeted daily practice compounds dramatically across Purdue's 16-week semester. Consistent daily sessions beat last-minute cramming every time." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA works at Purdue University West Lafayette.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your Purdue course notes into unlimited targeted practice questions. Track your mastery. Raise your GPA.</p>
