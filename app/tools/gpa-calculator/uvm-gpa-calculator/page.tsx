...
// ─── University of Vermont GPA Calculation ────────────────────────────────────
// UVM uses a standard 4.0 scale
// A+ = 4.0, A = 4.0, A- = 3.7
// B+ = 3.3, B = 3.0, B- = 2.7
// C+ = 2.3, C = 2.0, C- = 1.7
// D+ = 1.3, D = 1.0, D- = 0.7
// F  = 0.0
// P/NP (Pass/No Pass), W (withdrawal), I (incomplete), AU (audit) = not counted
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Dean's List: semester GPA ≥ 3.5 with 12+ graded credit hours
// Highest Honors: cumulative GPA 3.9+ at graduation
// High Honors: cumulative GPA 3.7–3.899 at graduation
// Honors: cumulative GPA 3.5–3.699 at graduation
...
const GRADE_POINTS: Record<string, number | null> = {
  ...
  "D+": 1.3, "D": 1.0, "D-": 0.7,
  "F":  0.0,
  "P": null, "NP": null, "W": null, "I": null, "AU": null,
};

const GRADE_OPTIONS = ["A+","A","A-","B+","B","B-","C+","C","C-","D+","D","D-","F","P","NP","W","I","AU"];
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 3.9) return "Highest Honors at UVM — the pinnacle of academic achievement at the University of Vermont. You're performing at an exceptional level. Keep it up, Catamount.";
  if (gpa >= 3.7) return "High Honors standing at UVM. This is an outstanding GPA that opens doors to top graduate programs, research fellowships, and competitive careers.";
  if (gpa >= 3.5) return "Dean's List at the University of Vermont. This reflects genuine academic excellence — keep building toward Honors recognition at graduation.";
  if (gpa >= 3.0) return "Solid academic standing at UVM. Push toward 3.5 to earn Dean's List recognition and qualify for graduation honors.";
  if (gpa >= 2.0) return "You're meeting UVM's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use UVM's Tutoring Center and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit UVM's Center for Academic Success and connect with your college advisor as soon as possible.";
}
...
const DB_NAME = "lunora_uvm_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. BIOL 1400)`} value={course.name}
...
export default function UVMGPACalculator() {
  ...
  const faqs = [
    {
      q: "How is GPA calculated at the University of Vermont?",
    ...
    {
      q: "What GPA do I need for Dean's List at UVM?",
    ...
    {
      q: "What cumulative GPA do I need for graduation honors at UVM?",
    ...
    {
      q: "What is the minimum GPA to stay in good standing at UVM?",
    ...
    {
      q: "What is an AU (audit) grade at UVM and does it affect my GPA?",
    ...
    {
      q: "How does UVM's Pass/No Pass option affect my GPA?",
  ...
  const honorThresholds = [
    { label: "Highest Honors",     range: "3.90 – 4.00", color: "#1a6e3c" },
    { label: "High Honors",        range: "3.70 – 3.89", color: "#1a6e3c" },
    { label: "Honors / Dean's",    range: "3.50 – 3.69", color: "#1e5fa8" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "UVM GPA Calculator", "description": "Free University of Vermont GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your UVM semester GPA and cumulative GPA. Supports UVM's full grading scale including A+, D-, P/NP, W, I, and AU exclusions.", "url": "https://lunora.app/tools/gpa-calculator/uvm-gpa-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · University of Vermont</div>
          </div>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 630, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>University of Vermont</strong> semester GPA and cumulative GPA instantly. Built for UVM's exact grading scale — full A+ through D−, P/NP, W, I, and AU exclusions all handled correctly.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "Full UVM scale with A+ & D−" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your UVM Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>P/NP, W, I, and AU grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at UVM</h2>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>UVM Honor Thresholds</span>
                </div>
                {honorThresholds.map(({ label, range, color }) => {
                  const isActive = getGPALabel(displayGPA) === label ||
                    (label === "Honors / Dean's" && displayGPA >= 3.5 && displayGPA < 3.7);
                  return (
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />UVM Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How University of Vermont GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 580, margin: "0 auto" }}>UVM uses a full 4.0 plus/minus scale including A+ and D−. P/NP, W, I, and AU grades are all excluded from your GPA calculation.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "UVM Grade Scale", sub: "4.0 max · Full plus/minus", body: "A+/A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · F = 0.0. P, NP, W, I, and AU are excluded.", formula: "grade_pts × credit_hrs" },
              ...
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at UVM, then divide by total graded credit hours attempted across your full academic record at the University of Vermont.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#1a6e3c", title: "UVM Honors", sub: "Semester + graduation", body: "Dean's List: 3.5+ semester (12+ hrs) · Graduation: Honors 3.50–3.69 · High Honors 3.70–3.89 · Highest Honors 3.90+. Pre-med students often target 3.5+ cumulative GPA.", formula: "GPA threshold → honor" },
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own UVM notes", body: "Upload your lecture slides, syllabi, and readings from any UVM course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From BIOL 1400 to CHEM 1400 to ECON 1400 — Lunora adapts to any University of Vermont course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every UVM exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, built from your actual UVM course material, compounds dramatically over a full semester. Early investment, outsized GPA return." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>UVM GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA works at the University of Vermont.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your UVM course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
