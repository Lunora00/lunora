...
// ─── OSU GPA Calculation ──────────────────────────────────────────────────────
// Ohio State University uses a standard 4.0 scale
// A  = 4.0, A- = 3.7
// B+ = 3.3, B  = 3.0, B- = 2.7
// C+ = 2.3, C  = 2.0, C- = 1.7
// D+ = 1.3, D  = 1.0, E  = 0.0  (OSU uses "E" not "F" for failure)
// S/U (Satisfactory/Unsatisfactory), W (withdrawal), I (incomplete) = not counted
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Dean's List: 3.5+ with 12+ graded hours
...
const GRADE_POINTS: Record<string, number | null> = {
  ...
  "D+": 1.3, "D":  1.0,
  "E":  0.0,
  "S": null, "U": null, "W": null, "I": null,
};

const GRADE_OPTIONS = ["A","A-","B+","B","B-","C+","C","C-","D+","D","E","S","U","W","I"];

function getGPALabel(gpa: number): string {
  if (gpa >= 4.0) return "President's List";
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 4.0) return "Perfect. You've earned Ohio State's President's List — the highest academic distinction OSU awards. Exceptional work.";
  if (gpa >= 3.7) return "Outstanding — Summa Cum Laude standing at Ohio State. This opens doors to the most competitive graduate programs and scholarships.";
  if (gpa >= 3.5) return "Dean's List territory. This GPA reflects real academic excellence and qualifies you for merit scholarships and honors programs at OSU.";
  if (gpa >= 3.0) return "Solid academic standing at Ohio State. Push toward 3.5 to earn Dean's List recognition and unlock scholarship opportunities.";
  if (gpa >= 2.0) return "You're meeting OSU's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use Ohio State's tutoring centers and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit OSU's Academic Success Center and connect with your college advisor as soon as possible.";
}
...
const DB_NAME = "lunora_osu_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. CHEM 1210)`} value={course.name}
...
export default function OSUCumulativeGPACalculator() {
  ...
  const faqs = [
    {
      q: "How is cumulative GPA calculated at Ohio State University?",
      a: "Ohio State University calculates cumulative GPA by dividing the total number of grade points earned across all letter-graded semesters by the total number of credit hours attempted. For each course, you multiply the grade point value by the credit hours — A = 4.0, A- = 3.7, B+ = 3.3, and so on — then sum those products. Divide by all letter-graded credits attempted. W (withdrawal), I (incomplete), S, and U grades are excluded. Like ASU, Ohio State uses 'E' rather than 'F' to denote a failing grade.",
    },
    {
      q: "What GPA do I need for Dean's List at Ohio State?",
      a: "To earn Dean's List recognition at Ohio State University, you generally need a semester GPA of 3.50 or higher while completing at least 12 graded credit hours in a semester. OSU also awards the President's List for students who achieve a perfect 4.0 GPA in a semester with 12 or more graded hours. Individual colleges at Ohio State may have their own honors criteria, so check with your specific school for exact requirements.",
    },
    {
      q: "What cumulative GPA do I need for graduation honors at OSU?",
      a: "Ohio State awards Latin honors based on cumulative GPA at graduation. Cum Laude requires a cumulative GPA of 3.50 to 3.69. Magna Cum Laude is awarded for a GPA of 3.70 to 3.89. Summa Cum Laude — the highest graduation honor at OSU — requires a cumulative GPA of 3.90 or above. You must also complete a minimum number of credit hours at Ohio State to be eligible for graduation honors.",
    },
    {
      q: "What is the minimum GPA to stay in good standing at Ohio State?",
      a: "Ohio State requires a minimum cumulative GPA of 2.0 to remain in good academic standing. Students who fall below 2.0 are placed on academic probation and must raise their GPA or risk academic suspension. Some OSU colleges and programs — including the Fisher College of Business and the College of Engineering — have higher minimum GPA requirements for continuation in their programs.",
    },
    {
      q: "Does Ohio State use 'F' or 'E' as a failing grade?",
      a: "Ohio State uses 'E' rather than 'F' to denote a failing grade, which is the same convention used by several other universities including ASU and Michigan. An E grade carries 0.0 quality points and is counted in your credit hours attempted, pulling your GPA down. This calculator includes the E grade option to match OSU's official grading scale.",
    },
    {
      q: "Do W (withdrawal) or I (incomplete) grades affect my OSU GPA?",
      a: "A W (withdrawal) grade at Ohio State does not affect your GPA — it records the drop but assigns no quality points. However, excessive withdrawals can affect your Satisfactory Academic Progress (SAP) and financial aid eligibility. An I (incomplete) grade is also excluded from GPA calculations initially, but if the coursework is not completed within the time OSU sets, it automatically converts to an E (failing), which does impact your GPA.",
    },
    {
      q: "How do I calculate my cumulative GPA across multiple semesters at Ohio State?",
      a: "To calculate your OSU cumulative GPA, add up the grade-point products from every letter-graded semester — (grade points × credit hours) for each course — across your entire academic record, then divide by total letter-graded credit hours attempted. The Cumulative GPA mode in this calculator makes it easy: enter your current GPA and credit hours from previous semesters, then add this semester's courses to see your updated cumulative GPA instantly.",
    },
    {
      q: "How can I raise my cumulative GPA at Ohio State?",
      a: "The fastest path to raising your cumulative GPA at OSU depends on how many total credits you've completed. In your first two years (under 60 credits), a single strong semester has an outsized effect. After 90+ credits, the denominator is so large that sustained improvement over multiple semesters is required. The highest-leverage strategies are: retaking courses where you earned a D or E (check OSU's grade forgiveness policy), loading up on high-credit courses you can ace, and building systematic daily study habits with tools like Lunora that generate targeted practice questions from your own course materials so nothing slips through the cracks before exams.",
    },
    {
      q: "Does Ohio State offer grade forgiveness or grade replacement?",
      a: "Ohio State University offers a Grade Forgiveness policy that allows undergraduate students to retake a course in which they received a D+, D, E, or EN and have the new grade replace the original in GPA calculations — though both grades remain on the transcript. There is a limit to the number of times grade forgiveness can be applied, so check with your academic advisor and OSU's registrar for the most current rules. Strategic use of grade forgiveness can meaningfully improve your cumulative GPA.",
    },
    {
      q: "How does OSU's semester GPA differ from my cumulative GPA?",
      a: "Your semester GPA at Ohio State reflects only the courses taken in a single semester, giving you a snapshot of that period's academic performance. Your cumulative GPA averages all graded credit hours across every semester at OSU. One strong semester raises cumulative GPA gradually; one weak semester has a similarly diluted effect as you accumulate more credits. This calculator lets you toggle between semester and cumulative GPA modes so you can see both.",
    },
  ];
  ...
  const honorThresholds = [
    { label: "President's List",       range: "4.00",        color: "#1a6e3c" },
    { label: "Summa Cum Laude",        range: "3.90 – 3.99", color: "#1a6e3c" },
    { label: "Magna Cum Laude",        range: "3.70 – 3.89", color: "#1e5fa8" },
    { label: "Cum Laude / Dean's List",range: "3.50 – 3.69", color: "#2d6e9e" },
    { label: "Good Standing",          range: "2.00 – 3.49", color: "#2d6e9e" },
    { label: "Academic Probation",     range: "0.00 – 1.99", color: "#c83232" },
  ];

  // OSU brand red
  const OSU_RED = "#203567";
    ...
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag-pill { display: inline-flex; align-items: center; gap: 6px; padding: 4px 14px; border-radius: 100px; background: rgba(32,53,103,0.07); border: 1px solid rgba(32,53,103,0.15); font-size: 11px; font-weight: 700; color: #203567; letter-spacing: 0.08em; text-transform: uppercase; font-family: 'DM Sans', sans-serif; }
        ...
        .score-card:hover { box-shadow: 0 8px 32px rgba(32,53,103,0.08); transform: translateY(-2px); }
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "OSU GPA Calculator", "description": "Free Ohio State University GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your OSU semester GPA and cumulative GPA across all semesters.", "url": "https://lunora.app/tools/gpa-calculator/osu-gpa-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color={OSU_RED} strokeWidth={2.5} />Free Tool · Ohio State University</div>
          </div>
          <h1 style={{ fontSize: "clamp(30px,5vw,56px)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.03em", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>
            OSU{" "}
            <span style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", color: OSU_RED }}>GPA Calculator</span>
          </h1>
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 600, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Ohio State University</strong> semester GPA and cumulative GPA instantly. Supports OSU's official grading scale including the E grade and W/I/S/U exclusions.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "OSU 4.0 scale with E grade" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
              <input type="text" placeholder="Semester (e.g. Spring 2026)" value={semesterLabel} onChange={e => setSemesterLabel(e.target.value)}
                style={{ padding: "8px 16px", border: "1.5px solid #eef0f8", borderRadius: 100, fontSize: 13, fontWeight: 600, color: OSU_RED, fontFamily: "'DM Sans', sans-serif", outline: "none", background: "white", minWidth: 200 }} />
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,53,103,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><BookOpen size={17} color={OSU_RED} strokeWidth={2} /></div>
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your OSU Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>W, I, S, and U grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
              ...
              <button onClick={addCourse} style={{ marginTop: 14, width: "100%", padding: "10px", border: "1.5px dashed #f0c0c0", borderRadius: 12, background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 600, color: OSU_RED, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "background 0.2s" }}
              ...
              <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <span style={{ fontSize: 12, color: "#888" }}>Credit hours counted toward GPA: <strong style={{ color: OSU_RED }}>{totalCredits}</strong></span>
                <span style={{ fontSize: 12, color: "#888" }}>Total grade points: <strong style={{ color: OSU_RED }}>{totalPoints.toFixed(2)}</strong></span>
              </div>
            </div>
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,53,103,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><TrendingUp size={17} color={OSU_RED} strokeWidth={2} /></div>
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at Ohio State</h2>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <div>
                    ...
                    <input type="number" min={0} max={4} step={0.001} value={prevGPA} onChange={e => setPrevGPA(Math.max(0, Math.min(4, Number(e.target.value) || 0)))}
                      style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #eef0f8", borderRadius: 10, fontSize: 15, fontWeight: 700, color: OSU_RED, fontFamily: "'DM Sans', sans-serif", outline: "none", background: "white" }} />
                  </div>
                  <div>
                ...
                <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <span style={{ fontSize: 12, color: "#888" }}>Total credits after this semester: <strong style={{ color: OSU_RED }}>{prevCredits + totalCredits}</strong></span>
                  <span style={{ fontSize: 14, fontWeight: 800, color: OSU_RED }}>New Cumulative GPA: {cumulativeGPA.toFixed(3)}</span>
                </div>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleReset} style={{ flex: 1, padding: "11px", border: "1.5px solid #eef0f8", borderRadius: 12, background: "white", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#888", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "border-color 0.2s, color 0.2s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = OSU_RED; (e.currentTarget as HTMLButtonElement).style.color = OSU_RED; }}
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: OSU_RED, letterSpacing: "0.08em", textTransform: "uppercase" }}>OSU Honor Thresholds</span>
                </div>
                {honorThresholds.map(({ label, range, color }) => {
                  const isActive = getGPALabel(displayGPA) === label || (label === "Cum Laude / Dean's List" && displayGPA >= 3.5 && displayGPA < 3.7);
                  return (
              ...
              <div style={{ textAlign: "left" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: OSU_RED, letterSpacing: "0.08em", textTransform: "uppercase", display: "block", marginBottom: 10 }}>Course Breakdown</span>
                {courses.map((c, i) => {
                  const pts = GRADE_POINTS[c.grade];
                    ...
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                          ...
                          <span style={{ fontSize: 12, fontWeight: 700, color: counted ? OSU_RED : "#ccc", flexShrink: 0, marginLeft: 6 }}>{c.grade}</span>
                        </div>
                        <div style={{ height: 5, background: "#eef0f8", borderRadius: 100, overflow: "hidden" }}>
                          <div style={{ height: "100%", background: counted ? OSU_RED : "#eee", width: counted ? `${((pts as number) / 4.0) * 100}%` : "0%", borderRadius: 100, transition: "width 0.6s ease" }} />
                        </div>
                      </div>
        ...
        {savedAttempts.length > 0 && (
          <div style={{ maxWidth: 1100, margin: "32px auto 0" }}>
            <button onClick={() => setShowHistory(h => !h)} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "'DM Sans', sans-serif" }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center" }}><History size={15} color={OSU_RED} strokeWidth={2} /></div>
              <span style={{ fontSize: 16, fontWeight: 700, color: "#1a1a1a" }}>Saved Semesters</span>
              <span style={{ fontSize: 12, color: "#aaa", background: "#fff0f0", padding: "2px 10px", borderRadius: 100, fontWeight: 600 }}>{savedAttempts.length}</span>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />OSU Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How Ohio State cumulative GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 560, margin: "0 auto" }}>OSU uses a 4.0 scale with one important difference — a failing grade is recorded as E, not F. W and I grades are excluded from GPA calculations.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: OSU_RED, title: "OSU Grade Scale", sub: "4.0 maximum · E for fail", body: "A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · E = 0.0. W, I, S, U are excluded. Note: OSU does not award A+ grades.", formula: "grade_pts × credit_hours" },
              { Icon: BookOpen, color: "#203567", title: "Semester GPA", sub: "Single semester", body: "For each letter-graded course, multiply the grade point value by credit hours. Sum those products, then divide by total graded credit hours attempted that semester.", formula: "Σ(pts × cr) / Σcr" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at Ohio State, then divide by total graded credit hours attempted across your entire academic record. This is the GPA on your transcript.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#1a6e3c", title: "OSU Honors", sub: "Graduation distinctions", body: "4.0 → President's List · 3.90+ → Summa Cum Laude · 3.70–3.89 → Magna Cum Laude · 3.50–3.69 → Cum Laude · 3.50+ semester → Dean's List · Below 2.0 → probation.", formula: "cumulative GPA → honor" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              Ohio State is one of the largest research universities in the country — and it's easy to drift behind without realizing it. Passive studying creates the feeling of preparation without the results. Lunora lets you upload your actual OSU course materials and generates unlimited active recall questions on exactly the content your professor will test. Whether you're grinding toward 3.5 for Dean's List or recovering from a rough semester, daily targeted practice is the highest-ROI study habit you can build.
            </p>
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: OSU_RED, fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for Free — No Credit Card <ArrowRight size={15} color={OSU_RED} strokeWidth={2.5} />
            </Link>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own OSU notes", body: "Upload your lecture slides, syllabi, and readings from any OSU course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From CHEM 1210 to MATH 1151 to COMM 2367 — Lunora adapts to any Ohio State course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every OSU exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, building from your actual course material, compounds dramatically over a full Ohio State semester. Early investment, outsized GPA return." },
            ...
            {otherGPATools.map(({ name, href }) => (
              <Link key={name} href={href} className="tool-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><GraduationCap size={13} color={OSU_RED} strokeWidth={2} /></div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: OSU_RED, letterSpacing: "-0.025em" }}>OSU GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#888", marginTop: 10 }}>Everything you need to know about how cumulative GPA works at Ohio State University.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your OSU course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
          <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "white", color: OSU_RED, fontWeight: 800, fontSize: 16, padding: "16px 40px", borderRadius: 100, textDecoration: "none" }}>
            Start learning free <ArrowRight size={16} color={OSU_RED} strokeWidth={2.5} />
          </Link>
        </div>
      </section>
