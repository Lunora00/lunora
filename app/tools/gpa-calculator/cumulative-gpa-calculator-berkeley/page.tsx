...
// ─── UC Berkeley GPA Calculation ────────────────────────────────────────────
// UC Berkeley uses a standard 4.0 scale (same as most UC campuses)
// A+ = 4.0, A = 4.0, A- = 3.7
// B+ = 3.3, B = 3.0, B- = 2.7
// C+ = 2.3, C = 2.0, C- = 1.7
// D+ = 1.3, D = 1.0, D- = 0.7
...
// Academic standing: Good standing ≥ 2.0
// Dean's List / High Distinction: ≥ 3.7 (per-semester, ≥12 units)
// Honors at graduation: Distinction ≥ 3.3 · High Distinction ≥ 3.7 · Highest Distinction ≥ 3.9
// Note: Berkeley uses "units" not "credit hours"
...
function getGPABg(gpa: number): string {
  ...
  if (gpa >= 3.3) return "rgba(30,95,168,0.07)";
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 3.9) return "Outstanding — you're on track for Highest Distinction honors at Berkeley. This puts you in the very top of your graduating class.";
  if (gpa >= 3.7) return "Excellent work. High Distinction standing reflects genuine mastery across Berkeley's rigorous curriculum.";
  if (gpa >= 3.3) return "Distinction territory. This cumulative GPA qualifies you for graduation honors and opens doors to top graduate programs.";
  if (gpa >= 3.0) return "Solid standing at Berkeley. Push toward 3.3 to reach Distinction honors and strengthen your graduate school profile.";
  if (gpa >= 2.0) return "You're meeting Berkeley's minimum requirements. Targeted effort on your lowest-performing courses can move this significantly.";
  if (gpa >= 1.0) return "Academic warning territory. Reach out to your college adviser and Berkeley's Student Learning Center now.";
  return "Immediate action needed. Visit Berkeley's Dean of Students office and speak with an academic adviser today.";
}

interface Course { id: string; name: string; units: number; grade: string; }
interface SavedAttempt { id: string; courses: Course[]; gpa: number; totalUnits: number; savedAt: number; label: string; semesterLabel: string; }

const DB_NAME = "lunora_berkeley_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function calcGPA(courses: Course[]): { gpa: number; totalUnits: number; totalPoints: number } {
  let totalUnits = 0; let totalPoints = 0;
  ...
  const gpa = totalUnits > 0 ? Math.round((totalPoints / totalUnits) * 1000) / 1000 : 0;
...
function calcCumulativeGPA(
  ...
  return Math.round(((currentGPA * currentUnits + newGPA * newUnits) / totalUnits) * 1000) / 1000;
}
...
function CourseRow({ course, onChange, onDelete, index }: {
  ...
  const contributed = pts !== null && pts !== undefined ? pts * course.units : null;
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 110px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input
        type="text"
        placeholder={`Course ${index + 1} (e.g. CS 61A)`}
        value={course.name}
      ...
      <input
        ...
        max={12}
        value={course.units}
        onChange={e => onChange({ ...course, units: Math.max(1, Math.min(12, Number(e.target.value) || 1)) })}
      ...
      >
        {GRADE_OPTIONS.map(g => <option key={g} value={g}>{g}{GRADE_POINTS[g] !== null && GRADE_POINTS[g] !== undefined ? ` (${GRADE_POINTS[g]?.toFixed(1)})` : " (no pts)"}</option>)}
      </select>
...
function newCourse(): Course {
  return { id: Date.now().toString(36) + Math.random().toString(36).slice(2), name: "", units: 3, grade: "A" };
}

export default function BerkeleyGPACalculator() {
  ...
  const [prevUnits, setPrevUnits] = useState(30);
  const [savedAttempts, setSavedAttempts] = useState<SavedAttempt[]>([]);
  const [justSaved, setJustSaved] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  const [scrolled, setScrolled] = useState(false);

  const { gpa, totalUnits, totalPoints } = calcGPA(courses);
  const cumulativeGPA = showCumulative ? calcCumulativeGPA(prevGPA, prevUnits, gpa, totalUnits) : gpa;
  ...
  const handleSave = async () => {
    const attempt: SavedAttempt = { id: Date.now().toString(36) + Math.random().toString(36).slice(2), courses: [...courses], gpa: displayGPA, totalUnits, savedAt: Date.now(), label: new Date().toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }), semesterLabel };
  ...
  const honorThresholds = [
    ...
    { label: "Distinction",         range: "3.30 – 3.69", color: "#1e5fa8" },
    { label: "Good Standing",       range: "2.00 – 3.29", color: "#2d6e9e" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "name": "Cumulative GPA Calculator Berkeley",
        "description": "Free UC Berkeley cumulative GPA calculator. Enter your courses, units, and grades to instantly calculate your semester GPA and cumulative GPA at UC Berkeley.",
        "url": "https://lunora.app/tools/gpa-calculator/cumulative-gpa-calculator-berkeley",
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · UC Berkeley</div>
          </div>
          <h1 style={{ fontSize: "clamp(30px,5vw,56px)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.03em", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", color: "#203567" }}>Berkeley</span>
          </h1>
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 600, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>UC Berkeley</strong> semester GPA and cumulative GPA instantly. Enter your courses, units, and grades — see your running cumulative average update in real time.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "UC Berkeley 4.0 scale" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              ...
              <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <span style={{ fontSize: 12, color: "#888" }}>Total units counted: <strong style={{ color: "#203567" }}>{totalUnits}</strong></span>
              ...
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>Enter your cumulative GPA and units before this semester</p>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  ...
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: "#555", display: "block", marginBottom: 6 }}>Previous Units Completed</label>
                    <input type="number" min={0} max={300} value={prevUnits} onChange={e => setPrevUnits(Math.max(0, Number(e.target.value) || 0))}
                      style={{ width: "100%", padding: "10px 14px", border: "1.5px solid #eef0f8", borderRadius: 10, fontSize: 15, fontWeight: 700, color: "#203567", fontFamily: "'DM Sans', sans-serif", outline: "none", background: "white" }} />
                  </div>
                </div>
                <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(32,53,103,0.04)", borderRadius: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: "#888" }}>Total units after this semester: <strong style={{ color: "#203567" }}>{prevUnits + totalUnits}</strong></span>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>Berkeley Honor Thresholds</span>
                </div>
                ...
                {courses.map((c, i) => {
                  ...
                  return (
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                      ...
                      <span style={{ fontSize: 11, color: "#aaa", width: 38, textAlign: "right", flexShrink: 0 }}>{counted ? `${c.units}u` : "P/NP"}</span>
                    </div>
                  );
                })}
                  ...
                  return (
                    <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12, border: "1.5px solid #eef0f8", background: "white" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#aaa" }}>{a.courses.length} courses · {a.totalUnits} units</div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />Berkeley Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How UC Berkeley GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 560, margin: "0 auto" }}>Berkeley uses a 4.0 scale with units instead of credit hours. Your cumulative GPA is the weighted average of all letter-graded coursework across every semester.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Grade Point Scale", sub: "4.0 maximum", body: "A+/A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · F = 0.0. P/NP courses do not count.", formula: "grade × units" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Semester GPA", sub: "Single-term average", body: "Multiply each course's grade points by its unit value. Sum those products for all letter-graded courses this semester. Divide by total units attempted to get your semester GPA.", formula: "Σ(pts × units) / Σunits" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "Running total at Berkeley", body: "Combine grade-point products and units from every previous semester with this semester's data. Your cumulative GPA is the weighted average across your entire Berkeley record.", formula: "(prev_pts + new_pts) / total" },
              { Icon: Trophy, color: "#1a6e3c", title: "Graduation Honors", sub: "Latin honors at Berkeley", body: "3.90+ → Highest Distinction · 3.70–3.89 → High Distinction · 3.30–3.69 → Distinction · 2.0 required for good standing · Below 2.0 triggers academic probation.", formula: "cumulative GPA → honors" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora Free — No Credit Card <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own Berkeley notes", body: "Upload your lecture slides, GSI notes, and readings. Lunora generates unlimited MCQ and free-response questions directly from your actual course material." },
              { Icon: Brain, title: "Built for Berkeley's hardest sequences", body: "CS 61A, Math 53, Chem 1A, Physics 7B — Lunora adapts to any course. Active recall from your own notes outperforms re-reading every time." },
              { Icon: TrendingUp, title: "Track mastery by topic across every course", body: "See your accuracy broken down by concept. Know exactly which topics need another pass before your midterm so nothing catches you off guard." },
              { Icon: Clock, title: "Short daily sessions that compound all semester", body: "20 minutes of targeted daily practice across a 15-week semester adds up fast. Consistent active recall builds the durable memory that Berkeley exams require." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>Cumulative GPA Calculator Berkeley — FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA and cumulative GPA work at UC Berkeley.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(28px,5vw,42px)", fontWeight: 800, color: "white", letterSpacing: "-0.025em", marginBottom: 16, fontFamily: "'DM Sans', sans-serif" }}>Stop estimating. Start studying smarter.</h2>
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your Berkeley course notes into unlimited targeted practice questions. Track your cumulative GPA toward Distinction.</p>
