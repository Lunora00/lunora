...
// ─── Clemson University GPA Calculation ────────────────────────────────────
// Clemson uses a standard 4.0 scale
// A  = 4.0
// B+ = 3.5, B = 3.0
// C+ = 2.5, C = 2.0
// D+ = 1.5, D = 1.0
// F  = 0.0
// NOTE: Clemson does NOT use plus/minus for A (no A+ or A-) and uses a
//       unique half-point increment scale (B+/C+/D+ = .5 above)
// S/U (Satisfactory/Unsatisfactory), W (withdrawal), I (incomplete),
// AU (audit) = not counted toward GPA
...
const GRADE_POINTS: Record<string, number | null> = {
  ...
  "D+": 1.5, "D": 1.0,
  "F":  0.0,
  "S": null, "U": null, "W": null, "I": null, "AU": null,
};

const GRADE_OPTIONS = ["A","B+","B","C+","C","D+","D","F","S","U","W","I","AU"];
...
function getGPALabel(gpa: number): string {
  ...
  if (gpa >= 3.5) return "Cum Laude / Dean's List";
...
function getGPAColor(gpa: number): string {
  if (gpa >= 3.9) return "#203567";
  if (gpa >= 3.7) return "#203567";
  if (gpa >= 3.5) return "#f66733";
...
function getGPABg(gpa: number): string {
  if (gpa >= 3.9) return "rgba(32,53,103,0.07)";
  if (gpa >= 3.7) return "rgba(32,53,103,0.07)";
  if (gpa >= 3.5) return "rgba(32,53,103,0.07)";
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 4.0) return "President's List — a perfect 4.0 at Clemson University. This is the pinnacle of academic achievement for Tigers. Exceptional work all around.";
  if (gpa >= 3.9) return "Summa Cum Laude standing at Clemson. This GPA opens doors to top graduate programs, competitive fellowships, and elite careers. You're performing at an elite level.";
  if (gpa >= 3.7) return "Magna Cum Laude territory at Clemson University. Outstanding academic achievement — keep building toward Summa distinction at graduation.";
  if (gpa >= 3.5) return "Cum Laude and Dean's List at Clemson. This reflects genuine excellence — push toward Magna and Summa recognition for graduation.";
  if (gpa >= 3.0) return "Solid academic standing at Clemson. Push toward 3.5 to earn Dean's List and qualify for graduation honors.";
  if (gpa >= 2.0) return "You're meeting Clemson's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use Clemson's Student Success Center and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit Clemson's Academic Success Center and connect with your college advisor as soon as possible.";
}
...
const DB_NAME = "lunora_clemson_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. BIOL 1030)`} value={course.name}
...
export default function ClemsonGPACalculator() {
  ...
  const faqs = [
    {
      q: "How is GPA calculated at Clemson University?",
      a: "Clemson University calculates GPA by dividing total grade points earned by total credit hours attempted with a letter grade. For each course you multiply the grade point value by credit hours — A = 4.0, B+ = 3.5, B = 3.0, C+ = 2.5, C = 2.0, D+ = 1.5, D = 1.0, F = 0.0. Clemson does not use A+, A-, B-, C-, or D- in its grading scale. S/U (Satisfactory/Unsatisfactory), W (withdrawal), I (incomplete), and AU (audit) grades are excluded entirely from GPA. Note that Clemson's scale uses half-point increments for the plus grades rather than the 0.3-increment system common at other universities.",
    },
    {
      q: "What GPA do I need for Dean's List at Clemson?",
      a: "To earn Dean's List recognition at Clemson University, you need a semester GPA of 3.5 or higher while completing at least 12 graded credit hours in that term. Clemson also awards a President's List distinction to students who achieve a perfect 4.0 GPA in a semester with 12 or more graded credit hours. Both honors appear on your official academic transcript and are recognized across all of Clemson's colleges.",
    },
    {
      q: "What cumulative GPA do I need for graduation honors at Clemson?",
      a: "Clemson University awards three tiers of Latin graduation honors. Cum Laude requires a cumulative GPA of 3.50 to 3.69. Magna Cum Laude is awarded for a GPA of 3.70 to 3.89. Summa Cum Laude — Clemson's highest graduation distinction — requires a cumulative GPA of 3.90 or above. You must complete a minimum number of credit hours at Clemson to be eligible, and some colleges may have additional requirements.",
    },
    {
      q: "What is the minimum GPA to stay in good standing at Clemson?",
      a: "Clemson University requires a minimum cumulative GPA of 2.0 to remain in good academic standing. Students who fall below 2.0 are placed on academic probation. Several Clemson programs — particularly engineering, nursing, and business tracks — have higher continuation GPA requirements for their specific majors. Always check with your department advisor for program-specific requirements.",
    },
    {
      q: "Does Clemson use A+, A-, or other plus/minus grades?",
      a: "Clemson University does not use A+, A-, B-, C-, or D- grades. The scale uses A, B+, B, C+, C, D+, D, and F. The plus grades (B+, C+, D+) are worth 3.5, 2.5, and 1.5 points respectively — a half-point above the corresponding letter grade. This is different from the 0.3-increment plus/minus system used at many other universities. This calculator is pre-configured for Clemson's exact scale, so your GPA will be accurate.",
    },
    {
      q: "What does an S/U (Satisfactory/Unsatisfactory) grade do to my GPA at Clemson?",
      a: "Courses taken under Satisfactory/Unsatisfactory grading at Clemson are excluded entirely from your GPA calculation. An S grade earns credit but adds no grade points; a U grade adds neither credit nor grade points. Clemson limits how many courses you can take S/U, and most major and core requirements must be completed for a letter grade. Using S/U strategically in electives can protect your GPA while letting you explore subjects outside your comfort zone.",
    },
    {
      q: "What does a W (withdrawal) do to my GPA at Clemson?",
      a: "A W grade at Clemson University does not affect your GPA — it simply records the withdrawal without assigning grade points. However, excessive W grades can raise concerns with graduate and professional school admissions committees and may affect your Satisfactory Academic Progress for financial aid. Clemson has specific add/drop and withdrawal deadlines each semester, so always consult the academic calendar and speak with your advisor before withdrawing from a course.",
    },
    {
      q: "How do I calculate my cumulative GPA across multiple semesters at Clemson?",
      a: "To calculate your Clemson cumulative GPA, sum the grade-point products (grade points × credit hours) from every letter-graded semester at Clemson, then divide by total letter-graded credit hours attempted. Use the Cumulative GPA mode in this calculator: enter your existing GPA and credit hour total from previous semesters, add this semester's courses, and your updated cumulative GPA appears instantly.",
    },
    {
      q: "What GPA do I need for Clemson's engineering or business programs?",
      a: "Clemson's College of Engineering, Computing and Applied Sciences and the Wilbur O. and Ann Powers College of Business both have competitive academic environments. While continuation GPA requirements vary by major, many engineering programs require a 2.0 or higher in technical courses and a 2.0 cumulative minimum to remain in the program. For competitive graduate school, research opportunities, or co-op placements, students in CECAS typically target 3.3+ and business students often aim for 3.5+. Your Clemson GPA is your starting point — use this calculator to track exactly where you stand.",
    },
    {
      q: "How can I raise my GPA at Clemson University?",
      a: "Raising your GPA at Clemson is most impactful early in your degree when the total credit hour denominator is smaller. Key strategies include: taking advantage of Clemson's course repeat policy for D or F grades, using S/U selectively for elective courses, targeting high-credit courses where you can excel, and replacing passive studying with active recall. Lunora lets you generate targeted practice questions from your actual Clemson course materials — from BIOL 1030 to CHEM 1010 to MTHSC 1080 — so you walk into every exam knowing your gaps are covered.",
    },
  ];
  ...
  const honorThresholds = [
    { label: "President's List",   range: "4.00",        color: "#203567" },
    { label: "Summa Cum Laude",    range: "3.90 – 4.00", color: "#203567" },
    { label: "Magna Cum Laude",    range: "3.70 – 3.89", color: "#203567" },
    { label: "Cum Laude / Dean's", range: "3.50 – 3.69", color: "#f66733" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "Clemson GPA Calculator", "description": "Free Clemson University GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your Clemson semester GPA and cumulative GPA. Supports Clemson's exact grading scale including A, B+, B, C+, C, D+, D, F and S/U, W, I, AU exclusions.", "url": "https://lunora.app/tools/gpa-calculator/clemson-gpa-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · Clemson University</div>
          </div>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 630, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Clemson University</strong> semester GPA and cumulative GPA instantly. Built for Clemson's exact grading scale — A, B+, B, C+, C, D+, D, F with S/U, W, I, and AU exclusions all handled correctly.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "Clemson's unique half-point scale" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your Clemson Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>S/U, W, I, and AU grades are automatically excluded from GPA calculations</p>
                </div>
              </div>
              ...
              <button onClick={addCourse} style={{ marginTop: 14, width: "100%", padding: "10px", border: "1.5px dashed #d0c8e8", borderRadius: 12, background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#203567", fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "background 0.2s" }}
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at Clemson</h2>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>Clemson Honor Thresholds</span>
                </div>
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />Clemson Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How Clemson University GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 580, margin: "0 auto" }}>Clemson uses a unique half-point scale — A = 4.0, B+ = 3.5, B = 3.0, and so on. No A+, A-, or minus grades. S/U, W, I, and AU are excluded.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Clemson Grade Scale", sub: "4.0 max · Half-point scale", body: "A = 4.0 · B+ = 3.5 · B = 3.0 · C+ = 2.5 · C = 2.0 · D+ = 1.5 · D = 1.0 · F = 0.0. No A+/A- or minus grades. S, U, W, I, and AU are excluded. Plus grades use .5 increments, not the .3 system used elsewhere.", formula: "grade_pts × credit_hrs" },
              { Icon: BookOpen, color: "#f66733", title: "Semester GPA", sub: "Single semester", body: "For each letter-graded course, multiply grade points by credit hours. Sum those products, then divide by total graded credit hours attempted that semester.", formula: "Σ(pts × cr) / Σcr" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at Clemson, then divide by total graded credit hours attempted across your full academic record at Clemson University.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#203567", title: "Clemson Honors", sub: "Semester + graduation", body: "President's List: 4.0 (12+ hrs) · Dean's List: 3.5+ (12+ hrs) · Graduation: Cum Laude 3.50–3.69 · Magna Cum Laude 3.70–3.89 · Summa Cum Laude 3.90+.", formula: "GPA threshold → honor" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              Clemson students face serious academic pressure — especially in engineering, business, and the sciences where a single grade in BIOL 1030 or CHEM 1010 can meaningfully shift your transcript. Passive studying is how students fall behind. Lunora lets you upload your actual Clemson course materials and generates unlimited active recall questions targeting exactly what your professors test. Whether you're building toward Summa Cum Laude or protecting your GPA in a tough semester, daily practice is how Tigers get the results they want.
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own Clemson notes", body: "Upload your lecture slides, syllabi, and readings from any Clemson course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From BIOL 1030 to CHEM 1010 to MTHSC 1080 — Lunora adapts to any Clemson course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every Clemson exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, built from your actual Clemson course material, compounds dramatically over a full semester. Early investment, outsized GPA return." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>Clemson GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA works at Clemson University.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your Clemson course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
