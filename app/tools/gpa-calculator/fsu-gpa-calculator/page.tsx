...
// ─── Florida State University GPA Calculation ─────────────────────────────────
// FSU uses a standard 4.0 scale
// A  = 4.0, A- = 3.7
// B+ = 3.3, B  = 3.0, B- = 2.7
// C+ = 2.3, C  = 2.0, C- = 1.7
// D+ = 1.3, D  = 1.0, D- = 0.7
// F  = 0.0
// Note: FSU does NOT award A+ grades
// S/U (Satisfactory/Unsatisfactory), W (withdrawal), I (incomplete), WF = not counted in GPA
// WF (Withdrawn Failing) DOES count as an F in GPA — treated as 0.0
// Cumulative GPA = Sum(grade_points × credit_hours) / Sum(credit_hours)
// Dean's List: semester GPA ≥ 3.5 with 12+ graded credit hours
// President's List: semester GPA = 4.0 with 12+ graded credit hours
// Summa Cum Laude: 3.9+ cumulative GPA at graduation
// Magna Cum Laude: 3.7–3.899 cumulative GPA at graduation
// Cum Laude: 3.5–3.699 cumulative GPA at graduation
...
const GRADE_OPTIONS = ["A","A-","B+","B","B-","C+","C","C-","D+","D","D-","F","WF","S","U","W","I"];
...
function getGPAMessage(gpa: number): string {
  if (gpa >= 4.0) return "Perfect — President's List at Florida State University. A 4.0 semester GPA is FSU's highest academic distinction. Exceptional work, Seminole.";
  if (gpa >= 3.9) return "Summa Cum Laude territory at FSU. You're performing at an elite level — this GPA qualifies you for the highest graduation honors and opens doors to top graduate programs.";
  if (gpa >= 3.7) return "Magna Cum Laude standing at Florida State. This reflects outstanding academic performance and positions you strongly for graduate school, professional programs, and competitive careers.";
  if (gpa >= 3.5) return "Dean's List at FSU. This reflects genuine academic excellence — keep building on it to reach graduation honors and strengthen your transcript.";
  if (gpa >= 3.0) return "Solid academic standing at Florida State University. Push toward 3.5 to earn Dean's List recognition and qualify for graduation honors.";
  if (gpa >= 2.0) return "You're meeting FSU's minimum requirements. Identify your weakest courses and build a targeted improvement plan this semester.";
  if (gpa >= 1.0) return "Academic warning territory. Use FSU's Academic Center for Excellence and advisor support now — early action makes the biggest difference.";
  return "Immediate intervention needed. Visit FSU's Student Success Center and connect with your college advisor as soon as possible.";
}
...
const DB_NAME = "lunora_fsu_gpa"; const DB_VERSION = 1; const STORE = "attempts";
...
function CourseRow({ course, onChange, onDelete, index }: { course: Course; onChange: (c: Course) => void; onDelete: () => void; index: number; }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 120px 36px", gap: 10, alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f0f3fa" }}>
      <input type="text" placeholder={`Course ${index + 1} (e.g. ENC 1101)`} value={course.name}
...
export default function FSUGPACalculator() {
  ...
  const faqs = [
    {
      q: "How is GPA calculated at Florida State University?",
      a: "Florida State University calculates GPA by dividing total grade points earned by total credit hours attempted with a letter grade. For each course you multiply the grade point value by credit hours — A = 4.0, A- = 3.7, B+ = 3.3, B = 3.0, and so on down to F = 0.0. Note that FSU does not award A+ grades. S/U (Satisfactory/Unsatisfactory), W (withdrawal), and I (incomplete) grades are excluded. However, WF (Withdrawn Failing) is treated the same as an F and does count against your GPA with 0.0 grade points.",
    },
    {
      q: "What GPA do I need for Dean's List at FSU?",
      a: "To earn Dean's List recognition at Florida State University, you need a semester GPA of 3.5 or higher while completing at least 12 graded credit hours in that term. FSU also awards the President's List for students who achieve a perfect 4.0 GPA with 12 or more graded credit hours in a semester. Both distinctions appear on your official academic transcript and are recognized across FSU's colleges including the College of Business and the FAMU-FSU College of Engineering.",
    },
    {
      q: "What cumulative GPA do I need for graduation honors at FSU?",
      a: "Florida State University awards Latin honors at graduation based on cumulative GPA. Cum Laude requires a cumulative GPA of 3.5 to 3.699. Magna Cum Laude is awarded for a GPA of 3.7 to 3.899. Summa Cum Laude — the highest graduation honor at FSU — requires a cumulative GPA of 3.9 or above. You must also complete a minimum number of credit hours at FSU to be eligible. Check with your college advising office for the most current eligibility requirements.",
    },
    {
      q: "What is a WF grade at FSU and does it affect my GPA?",
      a: "A WF (Withdrawn Failing) grade at Florida State University is one of the most important FSU-specific grading distinctions to understand. Unlike a regular W (withdrawal), which does not affect your GPA, a WF is assigned when you withdraw from a course after the deadline while your current grade is failing. A WF carries 0.0 grade points and is counted in your credit hours attempted — exactly like an F. This is a critical difference from many other universities. This calculator correctly treats WF as 0.0 grade points in GPA calculations.",
    },
    {
      q: "Does FSU award A+ grades?",
      a: "No — Florida State University does not award A+ grades. The highest grade at FSU is a plain A, worth 4.0 grade points. This is different from universities like IU, Berkeley, and ASU that include A+ on their official scales. This calculator reflects FSU's actual grading scale correctly — A+ is not included in the grade dropdown.",
    },
    {
      q: "What is the minimum GPA to stay in good standing at FSU?",
      a: "Florida State University requires a minimum cumulative GPA of 2.0 to remain in good academic standing. Students who fall below 2.0 are placed on academic probation. Continued GPA deficiency can lead to academic suspension. Several FSU programs — including the College of Business, the College of Nursing, and the College of Social Work — have higher continuation GPA requirements for their specific majors, so check with your program advisor.",
    },
    {
      q: "How does FSU's Satisfactory/Unsatisfactory (S/U) grading affect my GPA?",
      a: "Courses taken under Satisfactory/Unsatisfactory grading at FSU are excluded entirely from your GPA calculation. An S grade earns credit but adds no grade points; a U grade adds neither credit nor grade points. FSU limits the number of courses you can take S/U, and most major and general education requirements must be completed for a letter grade. Strategic use of S/U in elective courses can protect your GPA while exploring subjects outside your major.",
    },
    {
      q: "How do I calculate my cumulative GPA across multiple semesters at FSU?",
      a: "To calculate your FSU cumulative GPA, sum the grade-point products (grade points × credit hours) from every letter-graded semester at Florida State, then divide by total letter-graded credit hours attempted. Remember to include any WF grades as 0.0 grade points — they count just like F grades. The Cumulative GPA mode in this calculator handles everything: enter your previous GPA and credit hours, add this semester's courses, and your updated cumulative GPA appears instantly.",
    },
    {
      q: "Does FSU offer grade forgiveness or course repeats?",
      a: "Florida State University offers a Grade Forgiveness Policy that allows undergraduate students to repeat a course and have the second grade replace the first in GPA calculations, up to a set number of times. Both grades remain visible on the transcript, but only the repeated grade is factored into the GPA. There are limits on how many courses and how many total credit hours can be forgiven under this policy. Contact the FSU Registrar or your academic advisor for the current specifics, as rules can vary by college.",
    },
    {
      q: "How can I raise my GPA at Florida State University?",
      a: "Raising your GPA at FSU depends heavily on how many total credit hours you've completed. Early in your degree (under 60 hours), a single strong semester has an outsized effect. After 90+ hours, improvement requires sustained effort across multiple semesters. Key strategies include: taking advantage of FSU's Grade Forgiveness Policy for D or F grades, avoiding WF grades by withdrawing before the failing threshold when necessary, using S/U wisely for electives, and building daily active recall habits. Lunora lets you generate targeted practice questions from your actual FSU course materials — from ENC 1101 to ACG 2021 to BSC 2010 — so you walk into every exam knowing exactly where your knowledge gaps are.",
    },
  ];
  ...
  const honorThresholds = [
    ...
    { label: "Cum Laude / Dean's",  range: "3.50 – 3.69", color: "#2d6e9e" },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      {/* Schema.org */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebApplication", "name": "FSU GPA Calculator", "description": "Free Florida State University GPA calculator. Enter your courses, credit hours, and grades to instantly calculate your FSU semester GPA and cumulative GPA. Supports FSU's official grading scale including WF grades, no A+, and S/U exclusions.", "url": "https://lunora.app/tools/gpa-calculator/fsu-gpa-calculator", "applicationCategory": "EducationApplication", "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" }, "author": { "@type": "Organization", "name": "Lunora", "url": "https://lunora.app" } }) }} />
      ...
      <section style={{ padding: "160px 24px 56px", background: "linear-gradient(180deg,#f8f9fd 0%,#fff 100%)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
            <div className="tag-pill"><Zap size={11} color="#203567" strokeWidth={2.5} />Free Tool · Florida State University</div>
          </div>
          ...
          <p style={{ fontSize: 18, color: "#666", lineHeight: 1.7, maxWidth: 630, margin: "0 auto 20px" }}>
            Calculate your <strong style={{ color: "#1a1a1a" }}>Florida State University</strong> semester GPA and cumulative GPA instantly. Built for FSU's exact grading scale — no A+, WF grades counted correctly, and S/U exclusions handled automatically.
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {[{ Icon: CheckCircle, text: "FSU scale — WF counts as F" }, { Icon: History, text: "Saves your semesters" }, { Icon: Target, text: "Cumulative GPA mode" }].map(({ Icon, text }, i) => (
          ...
          <div>
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Your FSU Courses — {semesterLabel}</h2>
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>S/U, W, and I excluded · WF counts as F (0.0) · No A+ at FSU</p>
                </div>
              </div>
            ...
            {showCumulative && (
              <div className="score-card fade-up" style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 17, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Previous Academic Record at FSU</h2>
            ...
            <div style={{ borderRadius: 24, background: gpaBg, border: `2px solid ${gpaColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ borderRadius: 14, border: "1.5px solid #eef0f8", overflow: "hidden", background: "white", marginBottom: 20 }}>
                <div style={{ padding: "12px 16px", background: "#f8f9fd", borderBottom: "1px solid #eef0f8" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#203567", letterSpacing: "0.08em", textTransform: "uppercase" }}>FSU Honor Thresholds</span>
                </div>
                {honorThresholds.map(({ label, range, color }) => {
                  const isActive = getGPALabel(displayGPA) === label ||
                    (label === "Cum Laude / Dean's" && displayGPA >= 3.5 && displayGPA < 3.7);
                  return (
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><div className="tag-pill"><Info size={11} strokeWidth={2.5} />FSU Grading Guide</div></div>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How Florida State University GPA is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 580, margin: "0 auto" }}>FSU uses a 4.0 scale with no A+ grade. The critical FSU-specific rule: WF (Withdrawn Failing) counts as F with 0.0 grade points — unlike a regular W which is excluded.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "FSU Grade Scale", sub: "4.0 max · No A+ · WF = F", body: "A = 4.0 · A- = 3.7 · B+ = 3.3 · B = 3.0 · B- = 2.7 · C+ = 2.3 · C = 2.0 · C- = 1.7 · D+ = 1.3 · D = 1.0 · D- = 0.7 · F/WF = 0.0. S, U, W, and I are excluded. No A+ at FSU.", formula: "grade_pts × credit_hrs" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Semester GPA", sub: "Single semester", body: "For each letter-graded course (including WF), multiply grade points by credit hours. Sum those products, divide by total graded credit hours attempted that semester.", formula: "Σ(pts × cr) / Σcr" },
              { Icon: TrendingUp, color: "#1a6e3c", title: "Cumulative GPA", sub: "All semesters combined", body: "Add grade-point products across every semester at FSU, then divide by total graded credit hours attempted. WF grades from any semester are always included as 0.0 points.", formula: "(all pts) / (all credits)" },
              { Icon: Trophy, color: "#1a6e3c", title: "FSU Honors", sub: "Semester + graduation", body: "President's List: 4.0 semester · Dean's List: 3.5+ semester (12+ hrs) · Summa Cum Laude: 3.90+ cumulative · Magna Cum Laude: 3.70–3.89 · Cum Laude: 3.50–3.69.", formula: "GPA threshold → honor" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.65)", lineHeight: 1.75, marginBottom: 28 }}>
              FSU is a Top 20 public university with real academic demands — and the difference between a B and an A in courses like ACG 2021, BSC 2010, or ENC 1101 compounds across your entire transcript. Passive studying creates the illusion of preparation. Lunora lets you upload your actual FSU course materials and generates unlimited active recall questions on exactly what your professor tests. Whether you're chasing Dean's List or recovering from a WF, daily practice is how Seminoles build the GPA they want.
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Practice questions from your own FSU notes", body: "Upload your lecture slides, syllabi, and readings from any FSU course. Lunora generates unlimited MCQ and short-answer questions from your actual material — not generic textbook content." },
              { Icon: Brain, title: "Active recall beats passive review every time", body: "From ENC 1101 to ACG 2021 to BSC 2010 — Lunora adapts to any Florida State course. Active recall is the single most evidence-backed study technique for retention and exam performance." },
              { Icon: TrendingUp, title: "Track mastery topic by topic", body: "See exactly which concepts within each course you've mastered and which still need work. Walk into every FSU exam knowing your gaps are covered." },
              { Icon: Clock, title: "15 minutes a day compounds across 16 weeks", body: "A consistent 15-minute daily practice habit, built from your actual FSU course material, compounds dramatically over a full semester. Early investment, outsized GPA return." },
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>FSU GPA Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how GPA works at Florida State University.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your FSU course notes into unlimited targeted practice questions. Build daily habits that raise your cumulative GPA semester over semester.</p>
