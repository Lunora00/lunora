...
import {
  ...
  ArrowRight, RotateCcw, Info, Calculator,
} from "lucide-react";

// ─── AP Precalculus 2026 Scoring ──────────────────────────────────────────
// Section 1: 40 MCQ → 50% of composite · 1h 35min
//   Part A: 28 MCQ (no calculator) · 80min
//   Part B: 12 MCQ (graphing calculator required) · 15min
// Section 2: 4 FRQ → 50% of composite · 1h
//   Part A: 2 FRQ (no calculator) · 30min
//   Part B: 2 FRQ (graphing calculator required) · 30min
//   Each FRQ is worth 6 points → total FRQ raw = 24 pts
// Composite = (MCQ/40 × 50) + (FRQ_raw/24 × 50) → /100
// AP Score thresholds (approximate, College Board 2024–2025 data)
// AP Precalculus launched in 2023–2024 as a new course
// 5 rate ~16–18%, 3+ rate ~56–62%
...
const FRQ_CONFIG = [
  {
    label: "FRQ 1 — Polynomial & Rational Functions",
    max: 6,
    desc: "No calculator. Analyze polynomial or rational functions — zeros, end behavior, asymptotes, transformations, and modeling. Requires precise algebraic reasoning and function notation.",
  },
  {
    label: "FRQ 2 — Exponential & Logarithmic Functions",
    max: 6,
    desc: "No calculator. Model real-world contexts with exponential or logarithmic functions, interpret parameters, solve equations using logarithm properties, and justify reasoning analytically.",
  },
  {
    label: "FRQ 3 — Trigonometric & Polar Functions",
    max: 6,
    desc: "Graphing calculator required. Analyze sinusoidal or polar functions — period, amplitude, phase shift, modeling periodic phenomena, and interpreting trigonometric equations in context.",
  },
  {
    label: "FRQ 4 — Functions Involving Parameters, Vectors & Matrices",
    max: 6,
    desc: "Graphing calculator required. Work with parametric equations, vectors, matrix operations, or combinations of function types. Interpret behavior, model scenarios, and connect representations.",
  },
];

const FRQ_TOTAL_RAW = 24;
const MCQ_TOTAL     = 40;
...
function getScoreMessage(score: number): string {
  return ({
    5: "Outstanding — you've mastered polynomial, exponential, trigonometric, and parametric functions at the highest level.",
    4: "Strong result. A 4 on AP Precalculus demonstrates genuine command of function analysis and mathematical reasoning.",
    3: "You're in qualifying territory. Sharpen your FRQ justifications and trigonometric function modeling to push into a 4.",
    2: "You're building the foundation. Focus on exponential/logarithmic properties and sinusoidal modeling — these yield reliable points.",
    1: "Don't be discouraged. Consistent daily practice on function transformations and algebraic reasoning will turn this around.",
  } as Record<number, string>)[score] ?? "";
}
function getThresholdRange(score: number): string {
  return ({ 5: "≥75", 4: "60–74", 3: "45–59", 2: "30–44", 1: "0–29" } as Record<number, string>)[score] ?? "";
}

interface SavedAttempt { id: string; mcq: number; frq: number[]; composite: number; apScore: number; savedAt: number; label: string; }
const DB_NAME = "lunora_ap_precalc"; const DB_VERSION = 1; const STORE = "attempts";
...
export default function APPrecalculusScoreCalculator() {
  const [mcq, setMcq] = useState(26);
  const [frq, setFrq] = useState<number[]>([4, 4, 4, 4]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", color: "#1a1a1a", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "description": "Free AP Precalculus score calculator for 2026. Enter your MCQ and free response scores to instantly calculate your composite score and predicted AP score of 1–5.",
        "url": "https://lunora.app/tools/ap-precalculus-score-calculator",
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "FAQPage",
        "mainEntity": [
          { "@type": "Question", "name": "How is the AP Precalculus exam scored?", "acceptedAnswer": { "@type": "Answer", "text": "AP Precalculus has two sections each worth 50%. Section 1 is 40 MCQ in 1h 35min (Part A: 28 questions, no calculator; Part B: 12 questions, graphing calculator). Section 2 is 4 FRQs worth 6 pts each (24 raw total) in 1 hour (Part A: no calculator; Part B: graphing calculator). Composite out of 100 maps to AP score 1–5." } },
          { "@type": "Question", "name": "What composite score do I need for a 5 on AP Precalculus?", "acceptedAnswer": { "@type": "Answer", "text": "You generally need a composite score of approximately 75 or above to earn a 5 on AP Precalculus. The 5 rate is roughly 16–18%." } },
          { "@type": "Question", "name": "Does AP Precalculus give college credit?", "acceptedAnswer": { "@type": "Answer", "text": "Many colleges do not award credit for AP Precalculus since precalculus is considered a prerequisite course. However, some institutions use strong scores for placement into Calculus I. Always verify with each college's AP credit policy." } },
          { "@type": "Question", "name": "What topics are on the AP Precalculus exam?", "acceptedAnswer": { "@type": "Answer", "text": "AP Precalculus covers four units: polynomial and rational functions, exponential and logarithmic functions, trigonometric and polar functions, and functions involving parameters, vectors, and matrices." } },
          { "@type": "Question", "name": "How is the AP Precalculus FRQ scored?", "acceptedAnswer": { "@type": "Answer", "text": "Each of the four FRQs is worth 6 points, scored analytically by rubric. Points are awarded for correct setups, procedures, and interpretations. Partial credit is available." } }
        ]
      }) }} />
            ...
            <div className="score-card" style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>40 questions · <strong style={{ color: "#203567" }}>50%</strong> of total score · 1h 35min</p>
                </div>
              </div>
              <div style={{ marginBottom: 16, padding: "12px 14px", background: "rgba(32,53,103,0.04)", borderRadius: 10, border: "1px solid rgba(32,53,103,0.08)" }}>
                <p style={{ fontSize: 12, color: "#666", lineHeight: 1.6, margin: 0 }}>
                  <strong>Part A:</strong> 28 questions, no calculator (80 min) · <strong>Part B:</strong> 12 questions, graphing calculator required (15 min)
                </p>
              </div>
            ...
            <div className="score-card">
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "#aaa", marginTop: 2 }}>4 questions · 6 pts each · <strong style={{ color: "#203567" }}>50%</strong> of total score · 1h</p>
                </div>
              </div>
              ...
              <div style={{ marginTop: 16, marginBottom: 24, padding: "12px 14px", background: "rgba(74,124,199,0.06)", borderRadius: 10, border: "1px solid rgba(74,124,199,0.15)" }}>
                <p style={{ fontSize: 12, color: "#4a7cc7", lineHeight: 1.6, margin: 0 }}>
                  <strong>Part A (FRQ 1–2): no calculator · Part B (FRQ 3–4): graphing calculator required.</strong> Show all work — partial credit is awarded for correct setup and intermediate steps even if the final answer is wrong.
                </p>
              </div>
                ...
                {FRQ_CONFIG.map((q, i) => (
                  <div key={i}>
                    <div style={{ marginBottom: 6 }}>
                      ...
                      {i < 2 && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 700, color: "#888", background: "#f0f3fa", padding: "2px 8px", borderRadius: 100 }}>No calculator</span>}
                      {i >= 2 && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 700, color: "#4a7cc7", background: "rgba(74,124,199,0.1)", padding: "2px 8px", borderRadius: 100 }}>Graphing calculator</span>}
                      <div style={{ fontSize: 11, color: "#aaa", marginTop: 4, lineHeight: 1.4 }}>{q.desc}</div>
                    </div>
          ...
          <div>
            <div style={{ borderRadius: 24, background: scoreBg, border: `2px solid ${scoreColor}22`, padding: "32px 28px", marginBottom: 20, textAlign: "center", position: "sticky", top: 140 }}>
              ...
              <div style={{ textAlign: "left" }}>
                ...
                <BreakdownBar label="FRQ 1 — Poly" score={frq[0]} max={FRQ_CONFIG[0].max} color="#4a7cc7" />
                <BreakdownBar label="FRQ 2 — Exp/Log" score={frq[1]} max={FRQ_CONFIG[1].max} color="#4a7cc7" />
                <BreakdownBar label="FRQ 3 — Trig" score={frq[2]} max={FRQ_CONFIG[2].max} color="#4a7cc7" />
                <BreakdownBar label="FRQ 4 — Param" score={frq[3]} max={FRQ_CONFIG[3].max} color="#4a7cc7" />
      ...
      <section style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#1a1a1a", letterSpacing: "-0.025em", marginBottom: 12 }}>How the AP Precalculus score is calculated</h2>
            <p style={{ fontSize: 16, color: "#777", maxWidth: 540, margin: "0 auto" }}>MCQ and FRQ each count equally at 50% — making both algebraic fluency without a calculator and graphing calculator analysis equally important to your final score.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
            {[
              { Icon: BarChart2, color: "#203567", title: "Section 1: MCQ", sub: "50% of total score", body: "40 multiple choice questions in 1h 35min. Part A: 28 questions with no calculator test polynomial zeros, exponential equations, logarithm properties, and function transformations algebraically. Part B: 12 questions with a graphing calculator test trigonometric and parametric function analysis.", formula: "MCQ / 40 × 50" },
              { Icon: BookOpen, color: "#4a7cc7", title: "Section 2: FRQ", sub: "50% of total score", body: "4 free response questions in 1 hour — 6 points each. Part A (FRQ 1–2, no calculator): polynomial/rational and exponential/logarithmic functions. Part B (FRQ 3–4, graphing calculator): trigonometric/polar and parametric/vector/matrix functions. Show all work for partial credit.", formula: "FRQ_raw / 24 × 50" },
              { Icon: Calculator, color: "#2d6e9e", title: "Calculator Policy", sub: "Split by part", body: "AP Precalculus is unique in splitting both sections into calculator and no-calculator parts. A graphing calculator is required for MCQ Part B and FRQ Part B. Practicing both with and without a calculator is essential — no-calculator algebraic fluency is tested separately on Part A of both sections.", formula: "Part A + Part B" },
              { Icon: Trophy, color: "#1a7a4a", title: "AP Score", sub: "1–5", body: "Composite ≥75 → 5 · ≥60 → 4 · ≥45 → 3 · ≥30 → 2 · below → 1. The 5 rate is roughly 16–18% and the 3+ rate is approximately 56–62%. Cut scores may shift slightly each year based on exam difficulty and student performance.", formula: "composite → 1–5" },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }} className="promo-grid">
          <div>
            ...
            <Link href="/signin" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "white", color: "#203567", fontWeight: 800, fontSize: 15, padding: "14px 32px", borderRadius: 100, textDecoration: "none" }}>
              Try Lunora for AP Precalculus — Free <ArrowRight size={15} color="#203567" strokeWidth={2.5} />
            </Link>
            <div style={{ marginTop: 14, fontSize: 12, color: "rgba(255,255,255,0.35)" }}>No credit card needed</div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { Icon: Sparkles, title: "Unlimited AP Precalculus practice problems", body: "Generate MCQ and FRQ-style problems on polynomial functions, exponential models, sinusoidal analysis, and parametric equations — directly from your own notes and textbook." },
              { Icon: Brain, title: "Build no-calculator algebraic fluency", body: "MCQ Part A and FRQ Part A are completed without a calculator. Drill function analysis, equation solving, and transformation reasoning until the algebraic steps are automatic." },
              { Icon: TrendingUp, title: "Track mastery by function family", body: "See your performance broken down by unit — polynomial, exponential/log, trigonometric, parametric. Know exactly where points are being lost before exam day." },
              { Icon: Clock, title: "Master sinusoidal modeling for FRQ 3", body: "The trigonometric FRQ is where most students lose points. Practice writing sine and cosine function equations from real-world periodic data — temperature, tides, daylight hours — until the setup is automatic." },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
            {otherExams.map(({ name, href }) => (
              <Link key={name} href={href} className="exam-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(32,53,103,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Calculator size={13} color="#203567" strokeWidth={2} /></div>
                  <div>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: "#fafbfd" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#203567", letterSpacing: "-0.025em" }}>AP Precalculus Score Calculator FAQ</h2>
            <p style={{ fontSize: 15, color: "#8899bb", marginTop: 10 }}>Everything you need to know about how AP Precalculus is scored in 2026.</p>
          </div>
      ...
      <section style={{ padding: "80px 24px", background: "#203567" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.65)", marginBottom: 36, lineHeight: 1.65 }}>Turn your AP Precalculus notes into unlimited function-family practice problems. Track your progress to a 5.</p>
